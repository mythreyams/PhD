...
PolyDataPointerType CortexSurfaceReconstruct::surfaceReconstruction ( int label,int global_spacing )
{
	double spacing = 0;
	if (global_spacing > 0)
...
ImageDataPointerType CortexSurfaceReconstruct::createImageVolumeFromPolyData ( PolyDataPointerType poly, double bounds[6], int label, double zSpacing )
{
	if(poly->GetNumberOfCells())
	{
		...
		if(!zSpacing)
			...
			{
				case Pia:
					spacing[0] = spacing[1] = spacing[2] = 300;//50;
					break;
				...
				default:
					spacing[0] = spacing[1] = spacing[2] = 300;//1;
					break;
			}
		else
...
// 		std::flush(std::cout << "Calculating isosurface..." << std::endl);
        ...
        std::cout << " before adjustment " << bounds[0] << " " << bounds[1] << " " <<bounds[2] << " " <<bounds[3] << " " <<bounds[4] << " " <<bounds[5] << " " <<std::endl;
		for(int ii = 0; ii < 3; ++ii)
		{
			bounds[2*ii] = round(bounds[2*ii]) - zSpacing*additionalSections;
			bounds[2*ii+1] = round(bounds[2*ii+1]) + zSpacing*additionalSections;
		}
		/*double zOffset = zSpacing ? zSpacing : 50;
		if(zReversed)
		{
            
			bounds[4] += zOffset*additionalSections;	//open @ bottom for pia & WM
			bounds[5] -= zOffset*additionalSections;
		}
		else
		{
			bounds[4] -= zOffset*additionalSections;
			bounds[5] += zOffset*additionalSections;;	//open @ bottom for pia & WM
		}*/
		//std::cout << " after adjustment " << bounds[0] << " " << bounds[1] << " " <<bounds[2] << " " <<bounds[3] << " " <<bounds[4] << " " <<bounds[5] << " " <<std::endl;
