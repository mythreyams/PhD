...
void CortexReconstruction::startReconstruction(const char* outputFilename, int global_spacing)
{
// 	#ifdef DEBUG
...
// 	#endif
	if(parameters.piaFlag)
	{
		...
		piaSurface = pia3DReconstruct->surfaceReconstruction(Pia,global_spacing);
...
// 		piaSurface = surfaceReconstruction(Pia);
        ...
        piaWriter->writeVtkSurfaceFile(piaSurface);
		delete piaWriter;
	}
	...
	if(parameters.wmFlag)
	{
		...
		wmSurface = wm3DReconstruct->surfaceReconstruction(WhiteMatter,global_spacing);
...
// 		wmSurface = surfaceReconstruction(WhiteMatter);
        ...
        wmWriter->writeVtkSurfaceFile(wmSurface);
		delete wmWriter;
	}
