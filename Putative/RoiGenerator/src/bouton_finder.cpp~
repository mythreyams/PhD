...
void BoutonFinder::getNonBoutonPoints(ImageType::Pointer image)
{
...
//                               std::cout<< "1 "  << std::endl;
    ...
    //                                   std::cout<< "2 " << OtsuThreshold << std::endl;
                        ...
                        //if(z_pos == z)
                        if(((x_pos >= 0) &&  (x_pos < end_x)) && ((y_pos >= 0) &&  (y_pos < end_y)) && ((z_pos >= 0) &&  (z_pos < end_y)))
                        {
                            //std::cout<<"inside the loop"<<std::endl;
                            bool already_present = false;
                            // if this is not the bouton landmark point already
                            // then add it to the param list as transformed non bouton landmark
                            
                            for(int i=0; i< num_of_boutons; i++)
                            {
                                if(Utility::euclidean_distance(BoutonParamsArray[i].transformed_landmark,coords,3,1) == 0)
                                {
                                    // 
...
void BoutonFinder::writeBoutonLandmarkfile(void)
{
    ...
    for(unsigned int i=0; i < (NUM_RAYS) * 4; i=i+2)
    {
        ...
        double oldCoords[4], newCoords[4];
            ...
            LandMarkData << con_landmarks[0] << " " << con_landmarks[1] << " " << BoutonParamsArray[14].landmark[2] << std::endl;
