...
void Reader::readSpatialGraphFile(bool applyTransform)
{
	...
	if(!inputStream.fail())
	{
        ...
        static int reading_basal_dendrite_ID = false;
        static int reading_apical_dendrite_ID = false;
		static int reading_soma_ID = false;
        int cell_index = 0;
		int axon_index = 0;
        int dend_index = 0;
		int soma_index = 0;
        ...
        std::vector<int> tmpBasalDendriteIDs;
        std::vector<int> tmpApicalDendriteIDs;
		std::vector<int> tmpSomaIDs;
...
// 						}
					...
					if(parameters && currentLine.find("Cell_", 0) != std::string::npos)
                    {
                        ...
                        cell_index = atoi(tmp1);
                        std::cout<<"cell_index "<<cell_index<<std::endl;
                                            
					     // read whether it is a axon dend or soma 
					     
					     if(currentLine.find("_Axon", loc2+1) != std::string::npos)
					     {
					       reading_axon_ID = true;
					     }
					     else if(currentLine.find("_Dendrite_Basal", loc2+1) != std::string::npos)
					     {
					       reading_basal_dendrite_ID = true;
					     }
                         else if(currentLine.find("_Dendrite_Apical", loc2+1) != std::string::npos)
					     {
					       reading_apical_dendrite_ID = true;
					     }
					     else if(currentLine.find("_Soma", loc2+1) != std::string::npos)
					     {
					       reading_soma_ID = true;
					     }
                                            
                    }
                    if(parameters && reading_apical_dendrite_ID)
                    {
...
//                                                 
                            {
                                    ...
                                    int ftmp1 = atoi(tmp1);
                                    std::cout<<"Apical DendriteID int "<<ftmp1<<std::endl;
                                    tmpApicalDendriteIDs.insert(tmpApicalDendriteIDs.begin()+cell_index-1, ftmp1);         // amira files are columns after each other
                                    //std::cout<<"DendriteID int "<<ftmp1<<std::endl;
                                    delete [] tmp1;
                            }
                            
                            reading_apical_dendrite_ID = false;
                        }
                    }
                    if(parameters && reading_basal_dendrite_ID)
                    {
                        if(currentLine.find("Id", 0) != std::string::npos)
                        {
                            //std::cout<<"found id dendrite"<<std::endl;
...
//                                                 loc2 = currentLine.find_first_of(whitespace, loc1 + 1); //ignores last value: is always 1 anyways
                            ...
                            //while(loc2 != std::string::npos && count < 16)
                            {
                                    ...
                                    int ftmp1 = atoi(tmp1);
                                    std::cout<<"Basal DendriteID int "<<ftmp1<<std::endl;
                                    tmpBasalDendriteIDs.insert(tmpBasalDendriteIDs.begin()+cell_index-1, ftmp1);         // amira files are columns after each other
                                    //std::cout<<"DendriteID int "<<ftmp1<<std::endl;
                                    delete [] tmp1;
                            }
                            
                            reading_basal_dendrite_ID = false;
                        }
                    }
...
//                                                 loc2 = currentLine.find_first_of(whitespace, loc1 + 1); //ignores last value: is always 1 anyways
                            ...
                            {       
                                    ...
                                    tmpAxonIDs.insert(tmpAxonIDs.begin()+cell_index-1, ftmp1); 
                                    std::cout<<"axonID int "<<ftmp1<<std::endl;
                                    //transformation[count%4][count/4]= ftmp1;        // amira files are columns after each other
//                                                         loc3 = loc2;
//                                                         loc1 = currentLine.find_first_of(numbers, loc3);
...
//                                                 loc2 = currentLine.find_first_of(whitespace, loc1 + 1); //ignores last value: is always 1 anyways
                            ...
                            {       
                                    ...
                                    currentLine.copy(tmp1, loc2 - loc1+1, loc1);
                                        ...
                                        //std::cout<<"axonID "<<tmp1<<std::endl;
                                    int ftmp1 = atoi(tmp1);
                                    std::cout<<"soma int "<<ftmp1<<std::endl;
                                    tmpSomaIDs.insert(tmpSomaIDs.begin()+cell_index-1, ftmp1); 
                                    std::cout<<"somaID int "<<ftmp1<<std::endl;
                                    //transformation[count%4][count/4]= ftmp1;        // amira files are columns after each other
//                                                         loc3 = loc2;
//                                                         loc1 = currentLine.find_first_of(numbers, loc3);
//                                                         loc2 = currentLine.find_first_of(signs, loc3);
...
// 					delete [] tmp1, delete [] tmp2, delete [] tmp3;
		...
		std::vector< int >::iterator somaIDIter;
		
		for(axonIDIter = tmpAxonIDs.begin(); axonIDIter != tmpAxonIDs.end(); ++axonIDIter)
			inputSpatialGraph->addAxonID(*axonIDIter);
		
        for(dendriteIDIter = tmpBasalDendriteIDs.begin(); dendriteIDIter != tmpBasalDendriteIDs.end(); ++dendriteIDIter)
                inputSpatialGraph->addBasalDendriteID(*dendriteIDIter);
        
        for(dendriteIDIter = tmpApicalDendriteIDs.begin(); dendriteIDIter != tmpApicalDendriteIDs.end(); ++dendriteIDIter)
                inputSpatialGraph->addApicalDendriteID(*dendriteIDIter);
		
		for(somaIDIter = tmpSomaIDs.begin(); somaIDIter != tmpSomaIDs.end(); ++somaIDIter)
                        inputSpatialGraph->addSomaID(*somaIDIter);
...
void Reader::writeSpatialGraphFile2(const char* filename, AmiraSpatialGraph * given_spatial_graph)
{
//      std::list<std::list<Compartment * > >::iterator edge_list_it;
//      std::list<Compartment * >::iterator edge_it;
        std::vector< Vertex * >::iterator vertexIt;
...
//      NeuroMorphData << "define GRAPH " << /*amira_spatial_graph->vertice_list.size() +*/ /*amira_contour_graph->vertice_list.size() +*/ given_spatial_graph->getNumberOfVertices() + /*amira_spatial_graph->edge_list.size() +*/ /*amira_contour_graph->edge_list.size() +*/ inputSpatialGraph->getNumberOfEdges() << std::endl;
        NeuroMorphData << "define POINT " << number_of_edge_points << std::endl;
