...
Vertex::Vertex(Vertex * otherVertex)
{
	...
	this->distanceFromSoma = 0;
    this->distanceFromSomaEuclidean = 0;
    parentVertexID = 0;
    vertexLevel = 0;
}

Vertex::Vertex(Vertex * otherVertex, bool copyConnectedEdge)
{
	this->coordinates[0] = otherVertex->coordinates[0];
	this->coordinates[1] = otherVertex->coordinates[1];
	this->coordinates[2] = otherVertex->coordinates[2];
	...
	if(copyConnectedEdge)
	{
	  this->connectedEdges = otherVertex->connectedEdges;
	}
	else
	{
	  this->connectedEdges = new std::vector<Edge *>;
	}
}

Vertex::Vertex(Vertex * otherVertex, double _distanceFromSoma)
{
	this->coordinates[0] = otherVertex->coordinates[0];
	this->coordinates[1] = otherVertex->coordinates[1];
	this->coordinates[2] = otherVertex->coordinates[2];
	...
	this->connectedEdges = otherVertex->connectedEdges;
	this->edgeDirections = otherVertex->edgeDirections;
	this->distanceFromSoma = _distanceFromSoma;
    this->distanceFromSomaEuclidean = 0;
    parentVertexID = 0;
    vertexLevel = 0;
}
...
/*
Vertex::SetDistanceFromSoma(double _distanceFromSoma)
{
   this->distanceFromSoma = _distanceFromSoma;
};

double Vertex::GetDistanceFromSoma()
{
	return this->distanceFromSoma;
};*/
...
Edge::Edge(Edge * otherEdge)
{
    ...
    this->childID = otherEdge->childID;
     this->level = otherEdge->level;
	this->physicalLength = otherEdge->physicalLength;
    this->forward = otherEdge->forward;
    
}

Edge::Edge(Edge * otherEdge, int edgeConnectivity1, int edgeConnectivity2 )
{
	this->edgeConnectivity[0] = edgeConnectivity1; //otherEdge->edgeConnectivity[0];
	this->edgeConnectivity[1] = edgeConnectivity2; //otherEdge->edgeConnectivity[1];
	this->label = otherEdge->label;
	this->numEdgePoints = otherEdge->numEdgePoints;
	this->radius = otherEdge->radius;
	std::list< double >::const_iterator radiusListIt;
...
double Edge::segmentLength(double * point_within_edge, bool forward)
{
	if(!edgePointCoordinates.size())
		return 0;
    ...
    if(forward)
    {
        //edgePointCoordinates.reverse();
        double * lastPt = edgePointCoordinates.front();
        std::list< double * >::const_iterator ptIt = edgePointCoordinates.begin();
        ++ptIt;
        while(ptIt != edgePointCoordinates.end())
        {
            double * nextPt = (*ptIt);
            if(((lastPt)[0] == point_within_edge[0])
            &&((lastPt)[1] == point_within_edge[1])
            &&((lastPt)[2] == point_within_edge[2]))
            {
                break;
            }
            length += sqrt((nextPt[0] - lastPt[0])*(nextPt[0] - lastPt[0]) + (nextPt[1] - lastPt[1])*(nextPt[1] - lastPt[1]) + (nextPt[2] - lastPt[2])*(nextPt[2] - lastPt[2]));
    ...
    else
    {
        ...
        double * lastPt = edgePointCoordinates.back();
        std::list< double * >::reverse_iterator ptIt = edgePointCoordinates.rbegin();
        ++ptIt;
        while(ptIt != edgePointCoordinates.rend())
        {
            double * nextPt = (*ptIt);
            if(((lastPt)[0] == point_within_edge[0])
            &&((lastPt)[1] == point_within_edge[1])
...
void AmiraSpatialGraph::addAxonID( int axonID)
{
    AxonIDs.push_back(axonID);
};

void AmiraSpatialGraph::addBasalDendriteID( int dendriteID)
{
    BasalDendriteIDs.push_back(dendriteID);
};

void AmiraSpatialGraph::addApicalDendriteID( int dendriteID)
{
    ApicalDendriteIDs.push_back(dendriteID);
};

void AmiraSpatialGraph::addSomaID( int somaID)
{
    SomaIDs.push_back(somaID);
    std::cout<< "Added soma ID " << somaID << std::endl;
    
};

std::vector<int>* AmiraSpatialGraph::getAxonIDArray( )
{       
    return &AxonIDs;
...
std::vector<int>* AmiraSpatialGraph::getBasalDendriteIDArray( )
{
    return &BasalDendriteIDs;
};

std::vector<int>* AmiraSpatialGraph::getApicalDendriteIDArray( )
{
    return &ApicalDendriteIDs;
};

std::vector<int>* AmiraSpatialGraph::getSomaIDArray( )
{
    return &SomaIDs;
};


int AmiraSpatialGraph::getNumberofCells( )
{
    return SomaIDs.size();
};

int AmiraSpatialGraph::getAxonCellIndex( int ID)
{
    int id_index = 0xFFFF;
    
    for(int i = 0; i < AxonIDs.size(); i++ )
    {
        //std::cout << AxonIDs.at(i)<<std::endl;
        if(AxonIDs.at(i) == ID)
        {
...
int AmiraSpatialGraph::getDendriteCellIndex( int ID)
{
    int id_index = 0xFFFF;
    
    for(int i = 0; i < BasalDendriteIDs.size(); i++ )
    {
        if(BasalDendriteIDs.at(i) == ID)
        {
            return i;
        }
    }
    for(int i = 0; i < ApicalDendriteIDs.size(); i++ )
    {
        if(ApicalDendriteIDs.at(i) == ID)
        {
            return i;
        }
    }
...
int AmiraSpatialGraph::getSomaCellIndex( int ID)
{
    int id_index = 0xFFFF;
    //std::cout<< "In Get Soma Cell Index "<< std::endl;
    //std::cout << "given id = "<<ID<<std::endl;
    for(int i = 0; i < SomaIDs.size(); i++ )
    {
        if(SomaIDs.at(i) == ID)
        {
            //std::cout << "found cell # = "<<i<<std::endl;
            return i;
        }
    }
    //std::cout << "not found cell # "<<id_index<<std::endl;
    return id_index;
    
};

int AmiraSpatialGraph::getDendriteOrSomaCellIndex( int ID)
{
    int id_index = 0xFFFF;
    //std::cout<< "In Get Soma Cell Index "<< std::endl;
    //std::cout << "given id = "<<ID<<std::endl;
...
int AmiraSpatialGraph::getAxonDendriteOrSomaCellIndex( int ID)
{
    int id_index = 0xFFFF;
    std::cout<< "In Get Soma Cell Index "<< std::endl;
    std::cout << "given id = "<<ID<<std::endl;
...
void AmiraSpatialGraph::applyInverseTransformation(double ** inverse_transformation)
{
