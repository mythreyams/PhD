...
/****************************************************************************/
BoutonFinder::BoutonFinder()
{
  
};

BoutonFinder(const char * input, const char * output)
{
  
  
  
};
BoutonFinder::BoutonFinder(char* amira_file, char* image_folder_name, int start_index,int end_index, char* hxfile, char* outputpath, char* statoutputname, int *NumOfLandmarks )
{
  ...
  amiraReader = new Reader(amira_file, format.c_str());//"amira_file_withRad_centerline");  
...
//              ////std::cout << "Bouton at (x, y): ("<< x_pos << ", " << y_pos << ") Bright: "<< coords[LOCAL_BRIGHTNESS] << " Rad: " << coords[SURFACE] << std::endl;
        ...
        resampleSpatialGraph();
        
        //amiraReader->writeSpatialGraphFile2(resampled_graph);
        
        
        //std::cout<<"getting axon points"<<std::endl;
        // Get the axon points
        getAxonPoints(original_image, NumOfLandmarks);
...
/****************************************************************************/
void BoutonFinder::findOrientationOfAxon(int NumOfLandmarks, double * landmarkPoint, int numNieghbours,  double* angleslist)
{
...
//           }
	  ...
	  for(int i =0; i< NumOfLandmarks; i++)
	  {
	      dist = Utility::euclidean_distance(landmarkPoint,BoutonParamsArray[i].transformed_landmark,3,1);
			  
	      if(dist < 5)
	      {
		double * distCoord = new double[4];
		distCoord[3] = dist;
		distCoord[0] = BoutonParamsArray[i].transformed_landmark[0];
		distCoord[1] = BoutonParamsArray[i].transformed_landmark[1];
		distCoord[2] = BoutonParamsArray[i].transformed_landmark[2];
		points.push_back(distCoord);
		//points.push_back(&dist);
		
		//min_coord[3] = pos;
		
		////////std:://cout<<dist<<'\t'<<distCoord[0]<<'\t'<<distCoord[1]<<'\t'<<distCoord[2]<<std::endl;
	      }
	  }
          ...
          for(point_it = (++SortedPoints.begin()); point_it !=  SortedPoints.end(); point_it++)
          {
            ...
            SortedCoord[0] = (*point_it)[0];
            SortedCoord[1] = (*point_it)[1];
            SortedCoord[2] = (*point_it)[2];
            if((SortedCoord[0] >= (*SortedPoints.begin())[0]) && (SortedCoord[1] >= (*SortedPoints.begin())[1]) && (SortedCoord[2] >= (*SortedPoints.begin())[3])) 
            {
                PositivePoints.push_back(SortedCoord);
                // sort the points according to their distance from the landmark point
                ////////std:://cout<<'\t'<<"sorted positive"<<(*point_it)[0]<<'\t'<<(*point_it)[1]<<'\t'<<(*point_it)[2]<<'\t'<<(*point_it)[3]<<std::endl;
		 //writeListinAmiraCoords(SortedCoord, "/home/mythreya/Documents/output/S05_S28/diameter", "anglepositive", 0,1,1);
            }
            else
            {
                NegativePoints.push_back(SortedCoord);
		////////std:://cout<<'\t'<<"sorted negative"<<(*point_it)[0]<<'\t'<<(*point_it)[1]<<'\t'<<(*point_it)[2]<<'\t'<<(*point_it)[3]<<std::endl;
		 //writeListinAmiraCoords(SortedCoord, "/home/mythreya/Documents/output/S05_S28/diameter", "anglenegative", 0,1,1);
            }
        
          }
          
          writeListinAmiraCoords(PositivePoints, "/home/mythreya/Documents/output/S05_S28/diameter", "anglepositive", 0,1,1,1);
	  writeListinAmiraCoords(NegativePoints, "/home/mythreya/Documents/output/S05_S28/diameter", "anglepnegative", 0,1,1,1);
          ...
          for(positive_it = PositivePoints.begin(),negative_it = NegativePoints.begin(); i< numNieghbours; positive_it++,negative_it++,i++)
          {
              ...
              if((*positive_it)[0] != (*negative_it)[0])
              {
                  ...
                  // image is in the fourth quadrant. therefore - y
                  //angles[0] = atan((-(*positive_it)[1] - (*negative_it)[1]) / ((*positive_it)[0] - (*negative_it)[0]) ) * 180 / PI; 
                  //////std:://cout<<'\t'<<'\t'<<'\t'<<'\t'<<angles[0]<<std::endl;
              }
              else
              {
              ...
              {
                  angles[1] = atan2(abs((*positive_it)[2] - (*negative_it)[2]) , abs((*positive_it)[1] - (*negative_it)[1]) ) * 180 / PI; 
              }
              else
              {
                  angles[1] = 90;
              ...
              if((*positive_it)[0] != (*negative_it)[0])
              {
                  angles[2] = atan2(abs((*positive_it)[2] - (*negative_it)[2]) , abs((*positive_it)[0] - (*negative_it)[0]) ) * 180 / PI; 
              }
              else
              {
                  angles[2] = 90;
...
void  BoutonFinder::findRadius(Image2DType::Pointer image_plane, double * landmarkPoint,  double *  orientationList, double *rad_list, double  *contour_list)
{
    ...
    // which is the case when a dendrite is near by
        ...
        return_list = halfMaxRadiusCalculation(landmarkPoint[0],landmarkPoint[1],RAY_LEN_PER_DIRECTION,phi,image_plane);
        ...
        for(int i = 0; i < NUM_RAYS; i++)
        {
            ...
            return_list = halfMaxRadiusCalculation(landmarkPoint[0],landmarkPoint[1],RAY_LEN_PER_DIRECTION,phi,image_plane,rad_list,contour_list);
            return_list_it = return_list.begin();
            *radius = (*return_list_it)[0];
            return_list.pop_front();
            contour_list->push_back( *(return_list.begin()));
...
ImageType::Pointer BoutonFinder::getOriginalImage(char* path_name, int start_index, int end_index)
{
        ...
        strcpy(input_file, "proximity%03d.tif");
        
        
        
        std::string format = "proximity";
          //format += "_seg";
          format += "%03d.";
          format += "tif";
        ...
        input_reader->SetImageIO( itk::TIFFImageIO::New() );
...
void BoutonFinder::getAxonPoints(ImageType::Pointer image, int *NumOfLandmarks)
{
    ...
    // 
    int num_of_boutons = 0;
    
    std::vector< Edge * > * edges = resampled_graph->edgesPointer();

    unsigned int numOfEdges = edges->size();
    double end_x = (image->GetLargestPossibleRegion().GetSize(0)*XYSAMPLING);
    double end_y = (image->GetLargestPossibleRegion().GetSize(1)*XYSAMPLING);
    double end_z = (image->GetLargestPossibleRegion().GetSize(2)*ZSAMPLING);

    //std::cout<< end_x << " " << end_y << " " << end_z<<std::endl;

    //std::cout<< imageTranslation[X_COORD] << " " << imageTranslation[Y_COORD] << " " << imageTranslation[Z_COORD]<<std::endl;
    //std::cout<< end_x+imageTranslation[X_COORD] << " " << end_y+imageTranslation[Y_COORD] << " " << end_z+imageTranslation[Z_COORD]<<std::endl;
    ...
    double x_pos, y_pos, z_pos;
        ...
        {
// #pragma omp parallel for schedule(dynamic,1)
            for(long pos = 0; pos < numOfEdges; pos++)      //for each edge in list
            {
                Edge * currentEdge = edges->at(pos);
                std::list< double * >::iterator edge_it;
                std::list< double * >::iterator next_it;

                ///////std::cout<< currentEdge->label  << std::endl;
                //if( (currentEdge->label == Axon_1_ID) || (currentEdge->label == Axon_2_ID) || (currentEdge->label == Axon_3_ID ) ) 
                if( ( currentEdge->label == Axon_1_ID) || ( currentEdge->label == Axon_2_ID) || (currentEdge->label == Axon_3_ID ))
		//if(isAxon(currentEdge->label))
                {
		  
		    //////std::cout<<"inside axon ID test"<<std::endl;

                    //for every point along edge
                    for(edge_it = currentEdge->edgePointCoordinates.begin(); edge_it != currentEdge->edgePointCoordinates.end(); edge_it++)
                    {
    //                                   ////////////////std:://cout<< "2 " << OtsuThreshold << std::endl;
                        //////std::cout<<"inside edge iterator"<<std::endl;
                        double * coords = *edge_it;
                        ...
                        //if(z_pos == z)
                        if(((x_pos >= (0+X_OFFSET)) &&  (x_pos < (end_x-X_OFFSET))) 
                            && ((y_pos >= (0+Y_OFFSET)) &&  (y_pos < (end_y-Y_OFFSET))) 
                            && ((z_pos >= (0+Z_OFFSET)) &&  (z_pos < (end_z-Z_OFFSET))))
                        {
                            ////std::cout<<"inside bounds check"<<std::endl;
                            //std::cout<< "x " << x_pos << " y " << y_pos <<" z " << z_pos<< std::endl;
                            //std::cout<< x_pos+imageTranslation[X_COORD] << " " << y_pos+imageTranslation[Y_COORD] << " " << z_pos+imageTranslation[Z_COORD]<<std::endl;
...
// #pragma omp parallel for schedule(dynamic,1)
 ...
 //                                               ////////////////std:://cout<< "3 " << OtsuThreshold << std::endl;
                                                ...
                                                for(unsigned n=1; n<=nr_of_rays; n++)
                                                {
                                                        ...
                                                        //tmp = halfMaxRadiusCalculation(x0, y0, ray_length, nr_of_rays, threshold, n, image_plane_3);
...
////////////////std:://cout<< "Out of GetImagePlane" << std::endl;
        return image_plane;
};
/************************************************************************************/
...
void BoutonFinder::OtsuThresholdRadiusCalculation(double x0, double y0, Image2DType::Pointer image_plane, double *rad_array, double *contour_array, std::list<double*>* minCont, double angle, double * dia_front, double * dia_back)
{
    
  // calculate the thresold for the prox image plane
    unsigned int threshold;
    
    OtsuThresholdType::Pointer OtsuThreshold = OtsuThresholdType::New(); 
    
    OtsuThreshold->SetImage(image_plane);
    
    OtsuThreshold->Compute();
    
    threshold = (OtsuThreshold->GetThreshold());
    
  // send ray and calculate the radius
  ...
  ////////////////std:://cout<<"in sendray "<<std::endl;
    ...
    for(int j = 0; j < NUM_RAYS; j++)
    {
        ...
        double * min_front_coord = new double[2];
        double * min_back_coord = new double[2];
        ...
        for(int k = 0; k < rint(RAY_LEN_PER_DIRECTION/XYSAMPLING); k++)
        {
            ...
            else
            {
                ...
                grey_value = bilinearInterpolation(x_new, y_new, image_plane);
                
                if(grey_value <= threshold)
                {
                    // reached the threshold save this to be the boundary and exit the ray extension loop
                    front_coord[0] = x_new;
                    front_coord[1] = y_new;
...
//                 //coutour_index++;
        ...
        if(rad_ray < min_rad)
        {
            ...
            min_rad = rad_ray;
            dia_back[0] = back_coord[0]*XYSAMPLING;
            dia_back[1] = back_coord[1]*XYSAMPLING;
            dia_front[0] = front_coord[0]*XYSAMPLING;
            dia_front[1] = front_coord[1]*XYSAMPLING;
            //*dia_front = *front_coord;
...
void BoutonFinder::perpendicularOtsuThresholdRadiusCalculation(double x0, double y0, Image2DType::Pointer image_plane, double *rad_array, double angle, double * dia_front, double * dia_back, const char * outputpath,int zpos, bool *overlap)
{
    // Initialize the contact flags
    overlap[0] = false;
    overlap[1] = false;
    bool first_wrong_ray = true;
     ////////////std:://cout<< "overlap init"<< std::endl;
  ...
  // send ray and calculate the radius
    ...
    x0 = rint(x0/XYSAMPLING);
    y0 = rint(y0/XYSAMPLING);
    std::list< double * > landmark_profile;
    ...
    //for(int j = 0; j < NUM_RAYS; j++)
    {
         phi = (((angle))*PI/180 + PI/2);//j*(PI/NUM_RAYS) ;
	 
	 //////std:://cout<< "angle "<<angle<<"phi "<<phi*180/PI<<std::endl;
        ...
        // forward
        bool found_threshold_frwd = false;
        for(int k = 0; k < rint(RAY_LEN_PER_DIRECTION/XYSAMPLING)/0.1; k++)
        {
            
            double * profile_entry = new double[3];
            double * landmark_entry = new double[3];
            ...
            y_new = (y_f*cos(phi) - x_f*sin(phi));
            x_new = (y_f*sin(phi) + x_f*cos(phi));
            ...
            if(x_new <= 0.1 || y_new <= 0.1 || x_new >= image_plane->GetLargestPossibleRegion().GetSize(0)-1 || y_new >= image_plane->GetLargestPossibleRegion().GetSize(1)-1)
                        break;
            ...
            else
            {
                ...
                landmark_entry[0] = x_new;//*XYSAMPLING;
                landmark_entry[1] = y_new;//*XYSAMPLING;
                landmark_entry[2] = zpos;//*ZSAMPLING;
                ////////////////std:://cout<< landmark_entry[2]<< std::endl;
                //////////////////std:://cout<<'\t'<<x_f<<'\t'<<y_f<<'\t'<<grey_value<<'\t'<<phi<<std::endl;
                //////////////std:://cout<< landmark_entry[0]<<" " <<landmark_entry[1]<<" " <<landmark_entry[2]<<" " << std::endl;
                landmark_profile.push_back(landmark_entry);
                ...
                linear_profile_front.push_back(profile_entry);
                 ...
                 //////////////std:://cout<< landmark_entry[0]<<" " <<landmark_entry[1]<<" " <<landmark_entry[2]<<" " <<grey_value<<std::endl;
                ...
                if(grey_value <= threshold)
                {
                    ...
                    found_threshold_frwd = true;
                    
                   // now that we have found the edge of this axon lets proceed another pixel
		   // in the current direction and see if that exceeds the threshold
		   // if so it may be due to the fact that there is a contact with the dendrite
		   // therefore set the flag
		    x_f = x_f + NUMBER_OF_PIXELS_FOR_OVERLAP_RAY;
		    //y_f = y_f + 1;
		    ...
		    else
		    {
		      ...
		      if(grey_value > threshold)
		      {
			////////std:://cout<< "grey value "<<grey_value << " threshold "<< threshold<<std::endl;
			//////////////std:://cout<< "overlap front set bfor"<< std::endl;
			overlap[0] = true;
			//////std:://cout<< "front "<<" "<<front_coord[0]*XYSAMPLING + imageTranslation[0]<<" " <<front_coord[1]*XYSAMPLING + imageTranslation[1]<<" " << zpos*ZSAMPLING + imageTranslation[2]<< std::endl;
			//////std:://cout<< "centre "<<" "<<x0*XYSAMPLING + imageTranslation[0]<<" " <<y0*XYSAMPLING + imageTranslation[1]<<" " << zpos*ZSAMPLING + imageTranslation[2]<< std::endl;
			//////std:://cout<< "angle "<<angle<<"phi "<<phi*180/PI<<std::endl;
			//////////////std:://cout<< "overlap front set after"<< std::endl;
        ...
        if (found_threshold_frwd == false)
        {
            double min_gray = 0xffff;
            std::list< double * >::iterator profile_it; 
            // threshold has not been reached... select the half max for this one
            ...
            //////////////std:://cout<< "overlap front set false before"<< std::endl;
            overlap[0] = true;
	    //////////////std:://cout<< "overlap front set false after"<< std::endl;
	    //////std:://cout<< "front "<<" "<<front_coord[0]*XYSAMPLING + imageTranslation[0]<<" " <<front_coord[1]*XYSAMPLING + imageTranslation[1]<<" " << zpos*ZSAMPLING + imageTranslation[2]<< std::endl;
            //////std:://cout<< "centre "<<" "<<x0*XYSAMPLING + imageTranslation[0]<<" " <<y0*XYSAMPLING + imageTranslation[1]<<" " << zpos*ZSAMPLING + imageTranslation[2]<< std::endl;
	    //////std:://cout<< "angle "<<angle<<"phi "<<phi*180/PI<<std::endl;
        }
        ...
        y_f = (y0);
        //back
        bool threshold_found_bck = false;
        for(int l = 0; l < rint(RAY_LEN_PER_DIRECTION/XYSAMPLING)/0.1; l++)
        {
            double * profile_entry = new double[3];
            double * landmark_entry = new double[3];
            ...
            else
            {
                ...
                {
                    // reached the threshold save this to be the boundary and exit the ray extension loop
                    threshold_found_bck = true;
                    back_coord[0] = x_new;
                    back_coord[1] = y_new;
                    //////////////std:://cout<<std::endl;
                    //////////////std:://cout<< back_coord[0]<<" " <<back_coord[1]<<" "<<std::endl;
                    //////////////std:://cout<<std::endl;
		    ...
		    else
		    {
		      ...
		      {
			////////std:://cout<< "grey value "<<grey_value << " threshold"<< threshold<<std::endl;
			//////////////std:://cout<< "overlap back set before "<< std::endl;
			overlap[1] = true;
			//////std:://cout<< "back "<<" "<<back_coord[0]*XYSAMPLING + imageTranslation[0]<<" " <<back_coord[1]*XYSAMPLING + imageTranslation[1]<<" " << zpos*ZSAMPLING + imageTranslation[2]<< std::endl;
			//////std:://cout<< "centre "<<" "<<x0*XYSAMPLING + imageTranslation[0]<<" " <<y0*XYSAMPLING + imageTranslation[1]<<" " << zpos*ZSAMPLING + imageTranslation[2]<< std::endl;
			//////std:://cout<< "angle "<<angle<<"phi "<<phi*180/PI<<std::endl;
			//////////////std:://cout<< "overlap back set after"<< std::endl;
		      }
		    }
        ...
        if (threshold_found_bck == false)
        {
	  if(first_wrong_ray)
	  {
	     first_wrong_ray = false;
	     const char * filename = "ray_otsu_wrong";
	     //writeListinAmiraCoords(landmark_profile, outputpath, filename,1,1,1);
	  }
           // set the half max threshold 
            std::list< double * >::iterator profile_it; 
            double min_gray = 0xffff;
...
//                             ////////////////std:://cout<<'\t'<<front_coord[0]<<'\t'<<front_coord[1]<<'\t'<<max_gray/2<<'\t'<<found_threshold<<std::endl;
            ...
            overlap[1] = true;
	    //////////////std:://cout<< "overlap back set false"<< std::endl;
	    //////////std:://cout<< "back "<<" "<<back_coord[0]<<" " <<back_coord[1]<<" " << std::endl;
	    //////std:://cout<< "back "<<" "<<back_coord[0]*XYSAMPLING + imageTranslation[0]<<" " <<back_coord[1]*XYSAMPLING + imageTranslation[1]<<" " << zpos*ZSAMPLING + imageTranslation[2]<< std::endl;
	    //////std:://cout<< "centre "<<" "<<x0*XYSAMPLING + imageTranslation[0]<<" " <<y0*XYSAMPLING + imageTranslation[1]<<" " << zpos*ZSAMPLING + imageTranslation[2]<< std::endl;
	    //////std:://cout<< "angle "<<angle<<"phi "<<phi*180/PI<<std::endl;
       ...
       writeListinAmiraCoords(landmark_profile, outputpath, "1ray",1,1,1,1);
        // find the radius
        double rad_ray = XYSAMPLING * Utility::euclidean_distance(back_coord,front_coord,2,1);
        //////////////////std:://cout<<rad_ray<< std::endl;
//                 contour_array[//coutour_index] = back_coord[0];
...
//                 //coutour_index++;
        ...
        if(rad_ray < min_rad)
        {
            ...
            //////std:://cout<< "front"<< dia_front[0]<<" "<<dia_front[1]<<std::endl;
            //////std:://cout<< "back"<< dia_back[0]<<" "<<dia_back[1]<<std::endl;
...
/************************************************************************************/
void BoutonFinder::halfMaxRadiusCalculation(double x0, double y0, Image2DType::Pointer image_plane, double *rad_array, double *contour_array, std::list<double*>* minCont, double angle, double * dia_front, double * dia_back)
{
  ////////////////std:://cout<<"in sendray "<<std::endl;
    double min_rad = 0xffffffffffffffff;
    int coutour_index = 0;
...
void BoutonFinder::perpendicularHalfMaxRadiusCalculation(double x0, double y0, Image2DType::Pointer image_plane, double *rad_array, double *contour_array, std::list<double*>* minCont, double angle, double * dia_front, double * dia_back, const char * outputpath, int zpos)
{
  ...
  ////////////////std:://cout<<"in sendray "<<std::endl;
    ...
    //for(int j = 0; j < NUM_RAYS; j++)
    {
        phi = ((abs(angle))*PI/180 + (90*PI/180));//j*(PI/NUM_RAYS) ;
       ...
       // ////////////////std:://cout<<"lower "<<abs(angle)*PI/180 - (PI/NUM_RAYS)<<"Upper "<<abs(angle)*PI/180 + (PI/NUM_RAYS)<<std::endl;
        ...
        //if ( (phi >= ((abs(angle))*PI/180 + (89*PI/180) ) &&  (phi <=  ((abs(angle))*PI/180 + (91*PI/180)))))
        {
            //double  localPhi = phi;
            // this is a direction +/- 10 degree for 18 rays along the axon
            // therefore break
...
//                         ////////////////std:://cout<<'\t'<<x_f<<'\t'<<y_f<<'\t'<<grey_value<<'\t'<<phi<<std::endl;
         ...
         const char * filename = "ray";
         //filename += "_";
//          char *intStr = itoa(int(x0));
//          std::string str1 = std::string(intStr);
//          filename += str1;
//          filename += "_";
//          char *intStr1 = itoa(int(y0));
//          std::string str2 = std::string(intStr1);
//          filename += str2;
         //filename += to_string(y0);
         
        writeLandmarkFile(landmark_profile, outputpath, filename,1);
        ...
        if(found_threshold == false)
        {
            ...
            for(profile_it = linear_profile_front.begin(); profile_it != linear_profile_front.end(); profile_it++)
            {
                if((*profile_it)[2] < min_gray)
                {
                    ...
                    //rad_contour.push_back(front_coord);
                             //////////////std:://cout<<'\t'<<"false"<<front_coord[0]<<'\t'<<front_coord[1]<<'\t'<<max_gray/2<<'\t'<<found_threshold<<std::endl;
        ...
        if(found_threshold == false)
        {
            ...
            if(coutour_index<(NUM_RAYS*4))
            {
                ...
                coutour_index++;
                 ...
                 //////////////std:://cout<<std::endl;
                   //////////////std:://cout<<'\t'<<"false"<<back_coord[0]<<'\t'<<back_coord[1]<<'\t'<<max_gray/2<<'\t'<<found_threshold<<std::endl;
                    //////////////std:://cout<<'\t'<<//coutour_index<<std::endl;
            }
            //else
                //////////////////std:://cout<<"countour arr out of bound"<<std::endl;
...
/*SendRay(x0, y0, ray_legth, nr_of_rays, threshold, n, image_plane) sends a ray     */
/************************************************************************************/
std::list<double *>  BoutonFinder::halfMaxRadiusCalculation(double x0, double y0, float ray_length_per_direction, double phi, Image2DType::Pointer image_plane, double *rad_array, double *contour_array)
{
...
{
    std::vector< Edge * > * edges = resampled_graph->edgesPointer();
    std::vector< Vertex * > * vertices = resampled_graph->verticesPointer();
    int numOfEdges = edges->size();
    int numofVertices = vertices->size();
    ...
    resampled_graph->applyTransformation();
    
}



void BoutonFinder::writeLandmarkFile(std::list<double*>  dia_list, const char* outputPathname, const char* outputFilename, int with_translation)
{
...
// transformToWorldCoordinates(list);
  ...
  LandMarkData << ""                                    << std::endl;
  LandMarkData << ""                                    << std::endl;
  LandMarkData << "define Markers " << dia_list.size()     << std::endl;
  LandMarkData << ""                                    << std::endl;
  LandMarkData << "Parameters {"                        << std::endl;
  LandMarkData << "    NumSets 1,"                      << std::endl;
  LandMarkData << "    ContentType \"LandmarkSet\""     << std::endl;
  ...
  std::list< double * >::const_iterator diaIt = dia_list.begin();
        ...
        while(diaIt != dia_list.end())
        {
            //////////////////std:://cout<< "in loop" << std::endl;
                ...
                else if(with_translation == 2)
		{
		   LandMarkData << nextPt[X_COORD]*XYSAMPLING + imageTranslation[0]  << " " << nextPt[Y_COORD]*XYSAMPLING+imageTranslation[1] << " " << (nextPt[Z_COORD]*ZSAMPLING)+imageTranslation[2] << std::endl;
		}
...
void BoutonFinder::writeBoutonLandmarkFile(int NumOfLandmarks, const char* outputPathname, const char* outputFilename)
{
...
// transformToWorldCoordinates(list);
  ...
  LandMarkData << ""                                    << std::endl;
  LandMarkData << ""                                    << std::endl;
  LandMarkData << "define Markers " << boutonCount      << std::endl;
  LandMarkData << ""                                    << std::endl;
  LandMarkData << "Parameters {"                        << std::endl;
  LandMarkData << "    NumSets 1,"                      << std::endl;
  LandMarkData << "    ContentType \"LandmarkSet\""     << std::endl;
  ...
  for(int i = 0; i < NumOfLandmarks; i++)
  {
     if (BoutonParamsArray[i].which_cluster == 1)
     {
         ...
         LandMarkData << BoutonParamsArray[i].landmark[0] << " " << BoutonParamsArray[i].landmark[1] << " " << BoutonParamsArray[i].landmark[2] << std::endl;
...
void BoutonFinder::writeRawOverlapLandmarkFile(int NumOfLandmarks, const char* outputPathname, const char* outputFilename)
{
...
// transformToWorldCoordinates(list);
  ...
  for(int i = 0; i < NumOfLandmarks; i++)
  {
     if ((BoutonParamsArray[i].overlap[0] == 1) )
     {
         boutonCount++;
         
     }
     if ( (BoutonParamsArray[i].overlap[1] == 1))
     {
         boutonCount++;
  ...
  LandMarkData << "Markers { float[3] Coordinates } @1" << std::endl;
  LandMarkData << ""                                    << std::endl;
  LandMarkData << "# Data sectna aaye ho dum maro dumion follows"              << std::endl;
  LandMarkData << "@1"                                  << std::endl;
  ...
  for(int i = 0; i < NumOfLandmarks; i++)
  {
     if (BoutonParamsArray[i].overlap[0] == 1) 
     {
         ...
         LandMarkData << BoutonParamsArray[i].perpendicular_otsu_front[0]+ imageTranslation[0] << " " << BoutonParamsArray[i].perpendicular_otsu_front[1]+ imageTranslation[1] << " " << BoutonParamsArray[i].perpendicular_otsu_front[2]+ imageTranslation[2] << std::endl;
         
     }
     if (BoutonParamsArray[i].overlap[1] == 1)
     {
         
         LandMarkData << BoutonParamsArray[i].perpendicular_otsu_back[0]+ imageTranslation[0] << " " << BoutonParamsArray[i].perpendicular_otsu_back[1]+ imageTranslation[1] << " " << BoutonParamsArray[i].perpendicular_otsu_back[2]+ imageTranslation[2] << std::endl;
     }
...
void BoutonFinder::writeAllLandmarkFile(int NumOfLandmarks, const char* outputPathname, const char* outputFilename)
{
...
// transformToWorldCoordinates(list);
  ...
  LandMarkData << ""                                    << std::endl;
  LandMarkData << ""                                    << std::endl;
  LandMarkData << "define Markers " << NumOfLandmarks      << std::endl;
  LandMarkData << ""                                    << std::endl;
  LandMarkData << "Parameters {"                        << std::endl;
  LandMarkData << "    NumSets 1,"                      << std::endl;
  LandMarkData << "    ContentType \"LandmarkSet\""     << std::endl;
...
void BoutonFinder::writeListinAmiraCoords(std::list<double*> listinImageCoords, const char* outputPathname, const char* outputFilename, bool scale, bool translate, bool transform, bool overwrite )
{
...
// transformToWorldCoordinates(list);
  ...
  if(overwrite)
  {
     LandMarkData.open (format.c_str(), std::ofstream::out);
    
  }
  else
  { 
     LandMarkData.open (format.c_str(), std::ofstream::out | std::fstream::app);
  }
  ...
  for(list_it1 = listinImageCoords.begin() ; list_it1 != listinImageCoords.end(); list_it1++)
  {
	...
	LandMarkData <<  (list_it)[0] << " " <<  (list_it)[1] << " " <<  (list_it)[2] << std::endl;
        //std:://cout <<  (list_it)[0] << " " <<  (list_it)[1] << " " <<  (list_it)[2] << std::endl;
...
bool BoutonFinder::isAxon(int AxonID)
{
    for(int i = 0; i < resampled_graph->getAxonIDArray()->size(); i++)
    {
        if(AxonID == resampled_graph->getAxonIDArray()->at(i))
        {
            return true;
        }
...
bool BoutonFinder::isDendrite(int dendriteID)
{
    
    for(int i = 0; i < resampled_graph->getDendriteIDArray()->size(); i++)
    {
        if(dendriteID == resampled_graph->getDendriteIDArray()->at(i))
        {
            return true;
        }
...
void BoutonFinder::resampleSpatialGraph()
{
...
//     amiraInputGraphReader->readSpatialGraphFile(false);
  ...
  //             
    ...
    //amiraReader->writeSpatialGraphFile2(amira_graph);
