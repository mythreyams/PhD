...
/*                                                                          */
/****************************************************************************/
void Proximity::writeProximityImage(const char * output_file, double ** amira_transformation)
{
    std::cout<< " In Writing Proximity Image  "<< std::endl;
    
    //get the cropped roi from the original image
    ImageType::Pointer proximity_image = this->getProximityImage();
...
//     system(cd_command.c_str());
    ...
    writeImagePlanes(proximity_image, folder_name.c_str());
    
    writeLandmarkFile(folder_name.c_str());
    
    writeInfoFile(folder_name.c_str());
    
    writeHXFile( folder_name.c_str(),  amira_transformation);
...
******************************************************************************************/
ImageType::Pointer Proximity::getProximityImage()
{
        std::cout<< " In getProximityImage  "<< std::endl;
    ImageType::RegionType iterator_region = getIteratorRegion();
    ImageType::RegionType output_region = getOutputRegion();
...
/*                                                                          */
/****************************************************************************/
void Proximity::writeImagePlanes(ImageType::Pointer input_image, const char* folder_name)
{
...
void Proximity::writeLandmarkFile( const char* outputFilename)
{
...
void Proximity::writeInfoFile(const char* folder_name)
{
    ...
    ImageType::Pointer input_image = this->getProximityImage();
    ImageType::RegionType region = input_image->GetLargestPossibleRegion();
...
void Proximity::writeHXFile( const char* outputFoldername, double** transformation)
{
    std::cout<< " In writeHXFile  "<< std::endl;
        ImageType::Pointer prox_image = this->getProximityImage();        
        ImageType::RegionType output_region = this->getOutputRegion();
        ImageType::RegionType iterator_region = this->getIteratorRegion();
        ImageType::SizeType region_size;
...
//         proximity_label += "_";
        ...
        ListData << " "<< std::endl;
        ListData << "[ load ${SCRIPTDIR}/InfoFile.info ] setLabel {proximity.info}"<< std::endl;
        ListData << "       "<<std::endl;
        
        ListData << "proximity.info setTranslation " <<start[X_COORD]*XYSAMPLING<<" "<<start[Y_COORD]*XYSAMPLING<<" "<<start[Z_COORD]*ZSAMPLING<<" "<<std::endl;
        ...
        ListData << "proximity.info applyTransform "<<  std::endl;
        ListData << "proximity.info setTransform " <<transformation[0][0]<< " "<<transformation[1][0]<< " "<<transformation[2][0]<< " "<<transformation[3][0]<< " "<<transformation[0][1]<< " "<<transformation[1][1]<< " "<<transformation[2][1]<< " "<<transformation[3][1]<< " "<<transformation[0][2]<< " "<<transformation[1][2]<< " "<<transformation[2][2]<< " "<<transformation[3][2]<< " "<<transformation[0][3]<< " "<<transformation[1][3]<< " "<<transformation[2][3]<< " "<<transformation[3][3]<< " "<< std::endl;
        
        ListData << "[ load ${SCRIPTDIR}/landmark_locations.landmarkAscii ]"<< std::endl;
