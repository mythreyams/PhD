...
using namespace std;
#define PROX_PER_BLOCK		20
#define MAX_INTER_DISTANCE	4 // of the zone in microns; since the prox distance is also the same distance
#define NUMBER_OF_NIGHBOURING_POINTS    5//10 on either side
int pop_count = 0;
int main_push_cnt = 0;
int push_cnt = 0;
...
AxonDendriteProximityFinder::AxonDendriteProximityFinder( AmiraSpatialGraph * merged_graph,  const char *outputFilename, double max_dist, const char* somaLocationFileName/*, ImageType::Pointer nondecon_image*/)
     //5 arguments----for whole merged cell files without scaling errors
     {
         
          //std::cout<<"In constructor"<<std::endl;
         this->amira_graph = merged_graph; 
         //this->transformed_section_graph = section_graph;
          
         //get BoundingBox of merged graph
         this->bound_box = new double [6];
         this->amira_graph->getBoundingBox(bound_box);
         
         this->outputpath = outputFilename;
         
         this->max_dist = max_dist;
	 
	 //cell_specific_axon_list = new std::vector<Edge*>;
         //cell_specific_dendrite_list =  new std::vector<Edge*>;
	 
	 readLandmarksFile(somaLocationFileName, &(this->soma_locations_vector));
	 std::cout<< "Done Reading Soma Landmarks"<<std::endl;
	 
	 for(int i = 0; i < amira_graph->getNumberofCells() ; i++)
	 {
	   std::cout<< soma_locations_vector.at(i)[0] <<"," << soma_locations_vector.at(i)[1] <<","<< soma_locations_vector.at(i)[2]<<std::endl;
	 }
	 
	 getCellSpecificSpatialGraphs();
	 
	 std::cout<< "got single cells "<<std::endl;
	 
	 //std::getchar();
	 
	 //convertGraphIntoATree();
	 //std::cout<< "done converting into a tree "<<std::endl;
	 
         // write new spatial graphs
	 for(int i = 0; i < cell_specific_spatial_graps_vector.size(); i++)
	 {
	   std::string cellgraphname( "/cell_");
           //cellgraphname += "/cell_";
	   cellgraphname += Utility::inttochar(i+1);
	   //cellgraphname += ".am";
	   
	   std::ofstream cellgraphfile(cellgraphname.c_str());
	   cout<< "writing cell " << i+1 << endl;
	   writeGivenSpatialGraphFile(cellgraphname.c_str(), cell_specific_spatial_graps_vector.at(i));
         }
 	
 	 std::cout<< "wrote single cells "<<std::endl;
   
         ////std::cout<<"whole cell X : "<<bound_box[0]<<"  Y :  "<<bound_box[2]<<"  Z : "<<bound_box[4]<<std::endl;
         ////std::cout<<"whole cell X : "<<bound_box[1]<<"  Y :  "<<bound_box[3]<<"  Z : "<<bound_box[5]<<std::endl;  
         
         ////std::cout<<"constructor inputfilename: " << this->inputfilename <<std::endl;
         
         //read transformation info from transformed graph
         //readAmiraTransformations();
         
         
         //transform merged graph so that the section to be processed is in its original place
         //amira_graph->applyInverseTransformation(inverse_transformation);
         //transformed_graph->applyInverseTransformation(inverse_transformation);


//         transformed_graph->applyInverseTransformation(inverse_transformation);
         
         //get BoundingBox of transformed merged graph
         
         
         
         //this->transformed_graph->getBoundingBox(sectionboundingbox);
...
AxonDendriteProximityFinder::AxonDendriteProximityFinder(Reader * ameraReaderPointer, AmiraSpatialGraph * input_graph, AmiraSpatialGraph * merged_graph, ImageType::Pointer input_image, const char * inputFilename, int include_unknown, double max_dist, const char * manual_z_scale,const char *outputFilename/*, ImageType::Pointer nondecon_image*/ )
     //7 arguments ----- for whole cell files with manual z scaling
     {
...
//          original_image_nondecon->Update();
         ...
         this->offsetfile = manual_z_scale;
         this->max_dist = max_dist; 
         ...
         ////std::cout << "****************************************************************************************" << std::endl;
         ////std::cout << "* AXON-DENDRITE PROXIMITY FINDER V0.1                                                  *" << std::endl;
         ////std::cout << "* Programmed by AS                                                                     *" << std::endl;
         ////std::cout << "*                                                                                      *" << std::endl;
         ////std::cout << "* MODE: WHOLE-CELL (WITH MANUAL Z-SCALING)                                             *" << std::endl;
         ////std::cout << "*                                                                                      *" << std::endl;
         ////std::cout << "* PARAMETERS:                                                                          *" << std::endl;
         ////std::cout << "* inputFilename: " << this->inputfilename << std::endl;
         ////std::cout << "* offsetfile: " << this->offsetfile << std::endl;
         ////std::cout << "* eliminateDoubledProximites: " << this->eliminateDoubledProximites << std::endl;
         ////std::cout << "* include_unknown: " << this->include_unknown << std::endl;
         ////std::cout << "* numberOfPointsAway: " << this->numberOfPointsAway << std::endl;
         ////std::cout << "* max_dist: " << this->max_dist << std::endl;
         ////std::cout << "****************************************************************************************" << std::endl;
...
//          this->amira_graph = new AmiraSpatialGraph;       
//          this->amira_graph->mergeSpatialGraph(merged_graph);
         this->amira_graph = merged_graph;
         ...
         //read transformation info from transformed graph
         transformed_graph = input_graph;         
         readAmiraTransformations();
         
         //transform merged graph so that the section to be processed is in its original place         
         amira_graph->applyInverseTransformation(inverse_transformation);
         transformed_graph->applyInverseTransformation(inverse_transformation);
         bool scaled = manual_z_scaling(offsetfile, amira_graph, false);
         bool scaledsection = manual_z_scaling(offsetfile, transformed_graph, false);
...
//         transformed_graph->applyInverseTransformation(inverse_transformation);
         ...
         ////std::cout<<"whole cell X trans: "<<bound_box[0]<<"  Y :  "<<bound_box[2]<<"  Z : "<<bound_box[4]<<std::endl;
         ////std::cout<<"whole cell X trans: "<<bound_box[1]<<"  Y :  "<<bound_box[3]<<"  Z : "<<bound_box[5]<<std::endl;
         
         //write spatial graph file to check if transformation is correct
         const char * testname = "trans_test";         
         const char * dummyName = "bla";
         Reader*  CheckGraph = new Reader(dummyName, testname);
         CheckGraph->setSpatialGraph(amira_graph);
         CheckGraph->writeSpatialGraphFile();
         

         
         
         //get BoundingBox of section
         transformed_graph->getBoundingBox(sectionboundingbox);
         
         ////std::cout<<"section X : "<<sectionboundingbox[0]<<"  Y :  "<<sectionboundingbox[2]<<"  Z : "<<sectionboundingbox[4]<<std::endl;
         ////std::cout<<"section X : "<<sectionboundingbox[1]<<"  Y :  "<<sectionboundingbox[3]<<"  Z : "<<sectionboundingbox[5]<<std::endl;
...
AxonDendriteProximityFinder::AxonDendriteProximityFinder(Reader * ameraReaderPointer, AmiraSpatialGraph * input_graph, ImageType::Pointer input_image, const char * inputFilename, int include_unknown, int numOfPointsAway, double max_dist,const char *outputFilename/*, ImageType::Pointer nondecon_image*/)
     //6 arguments ---- for single section files
     {
         graphReaderPointer = ameraReaderPointer;
         this->inputfilename = inputFilename;
         original_image = input_image;
...
//          original_image_nondecon->Update();
         ...
         ////std::cout << "* Programmed by AS                                                                     *" << std::endl;
         ////std::cout << "*                                                                                      *" << std::endl;
         ////std::cout << "* MODE: SINGLE SECTION                                                                 *" << std::endl;
         ////std::cout << "*                                                                                      *" << std::endl;
         ////std::cout << "* PARAMETERS:                                                                          *" << std::endl;
         ////std::cout << "* inputFilename: " << this->inputfilename << std::endl;
         ////std::cout << "* eliminateDoubledProximites: " << this->eliminateDoubledProximites << std::endl;
        ...
        for(int ii = 0; ii < 4; ++ii)
        {
         ...
         this->amira_graph = new AmiraSpatialGraph;       
//          this->amira_graph->mergeSpatialGraph(merged_graph);
         this->amira_graph = input_graph;
            
          
           //
         this->bound_box = new double [6];
         this->amira_graph->getBoundingBox(bound_box);      
         this->amira_graph->getBoundingBox(sectionboundingbox); 
         ////std::cout<<"X : "<<bound_box[0]<<"  Y :  "<<bound_box[2]<<"  Z : "<<bound_box[4]<<std::endl;
         ////std::cout<<"X : "<<bound_box[1]<<"  Y :  "<<bound_box[3]<<"  Z : "<<bound_box[5]<<std::endl;
...
AxonDendriteProximityFinder::AxonDendriteProximityFinder(Reader * ameraReaderPointer, AmiraSpatialGraph * input_graph, ImageType::Pointer input_image, const char * inputFilename,const char *outputFilename/*, ImageType::Pointer nondecon_image*/)
     //3 arguments ---- for checking of murky regions
     {
         graphReaderPointer = ameraReaderPointer;
         this->inputfilename = inputFilename;
         original_image = input_image;
...
//          original_image_nondecon->Update();
         ...
         ////std::cout << "* Programmed by AS                                                                     *" << std::endl;
         ////std::cout << "*                                                                                      *" << std::endl;
         ////std::cout << "* MODE: CHECK AMBIGUOUS REGIONS                                                        *" << std::endl;
         ////std::cout << "*                                                                                      *" << std::endl;
         ////std::cout << "* PARAMETERS:                                                                          *" << std::endl;
         ////std::cout << "* inputFilename: " << this->inputfilename << std::endl;
         ////std::cout << "* eliminateDoubledProximites: " << this->eliminateDoubledProximites << std::endl;
...
//          this->amira_graph->mergeSpatialGraph(merged_graph);
         ...
         set_edge_lists(amira_graph, false);         
...
void AxonDendriteProximityFinder::readLandmarksFile(const char* somaLocationFileName, std::vector<double*> * soma_locations_vector_ptr)
{
  std::cout<< "soma file location "<< somaLocationFileName << std::endl;
  std::ifstream inputStream(somaLocationFileName);
  ...
  if(!inputStream.fail())
  {
    ...
    { 
      ...
      soma_locations_vector_ptr->push_back(tmp);
...
void AxonDendriteProximityFinder::sortProxDist( std::vector<Proximity *> *prox_list_unsorted, std::vector<Proximity *> *prox_list_sorted, bool set_linear_id )
{
    //std::cout<<"In sort"<<std::endl;
    std::vector<Proximity*>::iterator it;
    int linear_prox_id = 0;
	...
	linear_prox_id++;
        ...
        if(set_linear_id)
	{
	  prox_list_sorted->push_back( new Proximity(prox_list_unsorted->at(min_ind), this->original_image, linear_prox_id));
	}
	else
	{
...
{
    std::cout<<"In find min"<<std::endl;
    // iterate over the input list and find the min with
     //std::vector<Proximity*> outProxList;
    int num_of_neighbouring_points = NUMBER_OF_NIGHBOURING_POINTS;
    int max_interdist = MAX_INTER_DISTANCE;
    int start_index = 0;
    int end_index = 0;
    int prox_zone_cnt = 0;
    int local_min_cnt = 0;
    ...
    //std::cout<<num_of_neighbouring_points<<std::endl;
    //std::cout<<inputProxVector->size()<<std::endl;
    if(inputProxVector->size() > 0)
    {
//#pragma omp parallel for
        for(int i = 0; i < (inputProxVector->size()-1); i++)
        {
            ...
            if((dist > max_interdist) || (i == (inputProxVector->size()-1)))
            {
                int prox_zone_min_cnt = 0;
                int prox_zone_in_size = i-start_index+1;
                //std::cout<<"prox_zone_cnt "<<prox_zone_cnt<<std::endl;
                //std::cout<<"prox_zone_in_size "<<i-start_index+1<<std::endl;
                prox_zone_cnt++;
                
                // this is a separate prox zone, lets find the local minimas within this zone
                //double local_min = 0xffff;
                int min_indx = 0;
                //std::cout<< "start_index "<<start_index << std::endl;
                //std::cout<< "end_index "<<i << std::endl;
                // lets find local minimas within this prox zone
            
                if(prox_zone_in_size <= num_of_neighbouring_points )
                {
                    double local_min = 0xffff;
                    ...
                    Proximity * prox = new Proximity(inputProxVector->at(min_indx)->getAxonPoint(), inputProxVector->at(min_indx)->getDendritePoint(), inputProxVector->at(min_indx)->getDistance(), inputProxVector->at(min_indx)->Axon_ID, inputProxVector->at(min_indx)->Dendrite_ID, this->original_image,0,inputProxVector->at(min_indx)->axonDistanceFromSoma,inputProxVector->at(min_indx)->dendriteDistanceFromSoma);
                    outputProxVector->push_back(prox);
                    
                }
                else // start the slider to find mins within this segment
                {
                    ...
                    for(int k = start_index; k <= i ; k++)
                    {
                        ...
                        double local_min = 0xffff;
                        int negative_offset, positive_offset;
                        
                        if((k-start_index) < num_of_neighbouring_points)
                        {
                            negative_offset = k-start_index;
                            positive_offset = num_of_neighbouring_points;
                        }
                        else if((i-k) < num_of_neighbouring_points)
                        {
                            negative_offset = num_of_neighbouring_points;
                            positive_offset = i-k;
                        }
                        else
                        {
                        ...
                        // find out if this point is min within the given nighborhood
                        for(int l = (k -negative_offset); l < (k + positive_offset); l++)
                        {
                            //std::cout<< "negative_offset "<< negative_offset<< std::endl;
                            //std::cout<< "positive_offset "<< positive_offset<< std::endl;
                            //if(k != l)
                            {
                            if(inputProxVector->at(l)->getDistance() < local_min)
                            {
                                local_min = inputProxVector->at(l)->getDistance();
                            }
                        ...
                        if(inputProxVector->at(k)->getDistance() == local_min)
                        {
                            //std::cout<< "found local minima "<<local_min<<std::endl;
                            //std::cout<< "and k "<<k<<std::endl;
                            local_min_cnt++;
                            
                            prox_zone_min_cnt++;
                            // this prox zone is a local minima
                            // lets add this to the final prox zone
                            Proximity * prox = new Proximity(inputProxVector->at(k)->getAxonPoint(), inputProxVector->at(k)->getDendritePoint(), inputProxVector->at(k)->getDistance(), inputProxVector->at(k)->Axon_ID, inputProxVector->at(k)->Dendrite_ID, this->original_image,0,inputProxVector->at(k)->axonDistanceFromSoma,inputProxVector->at(k)->dendriteDistanceFromSoma);
                            outputProxVector->push_back(prox); 
                        }
            ...
            std::cout<<"prox_zone_out_size "<<prox_zone_min_cnt<<std::endl;
    ...
    //std::cout<< "in prox size "<<inputProxVector->size() <<std::endl;  
        
    //std::cout<< "out prox size "<<outputProxVector->size() <<std::endl;  
...
void AxonDendriteProximityFinder::RemoveDuplicateProximities(std::vector<Proximity*> *input_prox_vector)
{
   // loop through the current proximity vector and eliminate the duplicate proximities
  for(vector<Proximity*>::iterator landmark_it = input_prox_vector->begin(); landmark_it != input_prox_vector->end(); landmark_it++)
  {
    
    for(vector<Proximity*>::iterator landmark_it_inner = landmark_it + 1; landmark_it_inner != input_prox_vector->end(); )
    {
      //std::cout<< (*landmark_it)->getDendritePoint()[X_COORD] <<","<< (*landmark_it)->getDendritePoint()[Y_COORD] <<","<< (*landmark_it)->getDendritePoint()[Z_COORD] << std::endl;
      //std::cout<< (*landmark_it_inner)->getDendritePoint()[X_COORD] <<","<< (*landmark_it_inner)->getDendritePoint()[Y_COORD] <<","<< (*landmark_it_inner)->getDendritePoint()[Z_COORD] << std::endl;
      if(  ( (*landmark_it)->getDendritePoint()[X_COORD] == (*landmark_it_inner)->getDendritePoint()[X_COORD] )
	  &&( (*landmark_it)->getDendritePoint()[Y_COORD] == (*landmark_it_inner)->getDendritePoint()[Y_COORD] )
	  &&( (*landmark_it)->getDendritePoint()[Z_COORD] == (*landmark_it_inner)->getDendritePoint()[Z_COORD] ) )
      {
	std::cout<< "got in" << std::endl;
	if(landmark_it_inner != input_prox_vector->end())
	{
	    landmark_it_inner = input_prox_vector->erase(landmark_it_inner);
	}
...
void AxonDendriteProximityFinder::writeProxAxonLandmarks( const char* outputFilename, std::vector<Proximity*> *sorted_prox_vector)
{
    //std::cout<< " In writeLandmarkFile  "<< std::endl;
  ////std::cout<< "in writeLandmarkFile" << std::endl;
...
//   format += "/landmark_locations.landmarkAscii";
  ...
  for(int i=0; i < sorted_prox_vector->size(); i++)
  {
    LandMarkData << sorted_prox_vector->at(i)->getAxonPoint()[X_COORD] << " " << sorted_prox_vector->at(i)->getAxonPoint()[Y_COORD] << " " << sorted_prox_vector->at(i)->getAxonPoint()[Z_COORD] << std::endl;
  } 
  
  LandMarkData.close();
};

void AxonDendriteProximityFinder::writeProxDendLandmarks( const char* outputFilename, std::vector<Proximity*> *sorted_prox_vector)
{
    //std::cout<< " In writeLandmarkFile  "<< std::endl;
  ////std::cout<< "in writeLandmarkFile" << std::endl;
...
//   format += "/landmark_locations.landmarkAscii";
  ...
  for(int i=0; i < sorted_prox_vector->size(); i++)
  {
    LandMarkData << sorted_prox_vector->at(i)->getDendritePoint()[X_COORD] << " " << sorted_prox_vector->at(i)->getDendritePoint()[Y_COORD] << " " << sorted_prox_vector->at(i)->getDendritePoint()[Z_COORD] << std::endl;
...
{
   std::cout<<"In writeProxXls"<<std::endl;
    bool file_exsists = 0;
...
//     } 
    ...
    xlsParamWriter.open (s1.c_str(),   std::fstream::out /*| std::fstream::app*/ );
    ...
    //if(!file_exsists)
    {
        ...
        <<"Distance"<<","<<"From Axon of Cell #"<<","<<"To Dendrite of Cell #"<<","<<"Axon Distance From Soma"<<","<<"Dendrite Distance From Soma"<<","<<"Section #"<<","
	<<"Overlap Location X"<<","<<"Overlap Location Y"<<","<<"Overlap Location Z"<<","
	<<"Bouton Location X"<<","<<"Bouton Location Y"<<","<<"Bouton Location Z"<<","
	<<"Overlap Status"<<","<<"Bouton Status"<<","<<"Putative Contact Status"<<'\n';
...
//         <<"Number of Boutons"<<","<<"Number of Putative Contacts"<<'\n';
    ...
    for(int i = 0; i<sorted_prox_vector->size(); i++)
    {
        ...
        xlsParamWriter<<i+1/*sorted_prox_vector->at(i)->getLinearID()*/<<","<<sorted_prox_vector->at(i)->getRealCentroid()[0]<<","<<sorted_prox_vector->at(i)->getRealCentroid()[1]<<","<<sorted_prox_vector->at(i)->getRealCentroid()[2]<<","
        <<sorted_prox_vector->at(i)->getDistance()<<","
        <<(amira_graph->getAxonCellIndex(sorted_prox_vector->at(i)->Axon_ID))+1<<","<<(amira_graph->getDendriteOrSomaCellIndex(sorted_prox_vector->at(i)->Dendrite_ID))+1<<","
	<<sorted_prox_vector->at(i)->axonDistanceFromSoma<<","<<sorted_prox_vector->at(i)->dendriteDistanceFromSoma<<'\n';
...
void AxonDendriteProximityFinder::writeProxXlsWithAxonDend(const char* filename, std::vector<Proximity*> *sorted_prox_vector)
{
   std::cout<<"In writeProxXls"<<std::endl;
    bool file_exsists = 0;
...
//         <<"Number of Boutons"<<","<<"Number of Putative Contacts"<<'\n';
    ...
    for(int i = 0; i<sorted_prox_vector->size(); i++)
    {
        ...
        /*xlsParamWriter<<i+1<<","<<sorted_prox_vector->at(i)->getAxonPoint()[0]<<","<<sorted_prox_vector->at(i)->getAxonPoint()[1]<<","<<sorted_prox_vector->at(i)->getAxonPoint()[2]<<","
        <<sorted_prox_vector->at(i)->getDistance()<<","
	...
	<<sorted_prox_vector->at(i)->axonDistanceFromSoma<<","<<sorted_prox_vector->at(i)->dendriteDistanceFromSoma<<'\n';
        ...
        /*xlsParamWriter<<i+1<<","<<sorted_prox_vector->at(i)->getDendritePoint()[0]<<","<<sorted_prox_vector->at(i)->getDendritePoint()[1]<<","<<sorted_prox_vector->at(i)->getDendritePoint()[2]<<","
        <<sorted_prox_vector->at(i)->getDistance()<<","
        <<(amira_graph->getAxonCellIndex(sorted_prox_vector->at(i)->Axon_ID))+1<<","<<(amira_graph->getDendriteCellIndex(sorted_prox_vector->at(i)->Dendrite_ID))+1<<","
	<<sorted_prox_vector->at(i)->axonDistanceFromSoma<<","<<sorted_prox_vector->at(i)->dendriteDistanceFromSoma<<'\n';*/
...
void AxonDendriteProximityFinder::getCellSpecificSpatialGraphs()
{
   ...
   { 
      ...
      else if(isDendrite(connetedEdgeLabel))
      {
   ...
   for(int edgecount = 0; edgecount < edges->size(); edgecount++)    
   {
      ...
      else if(isDendrite(edges->at(edgecount)->label))
      {
...
void AxonDendriteProximityFinder::convertGraphIntoATree()
{
  
   double min_dist_around_soma = 50;
   ...
   for(int i = 0; i < cell_specific_spatial_graps_vector.size(); i++)
   {
      ...
      std::vector< double * > nodes_near_soma;
      
     
   
      // Find the nearest nodes to the root (soma)
      // They should be nodes with just one edge connected 
      for (int j = 0; j < present_graph->getNumberOfVertices(); j++)
      {
	
	//std::cout<< "init dist from soma "<<vertices->at(j)->distanceFromSoma << std::endl;
	
	 double dist = Utility::euclidean_distance(soma_locations_vector.at(i), vertices->at(j)->coordinates, 3, 1);
	 if( /*( vertices->at(j)->getConnectedEdges()->size() == 1 ) &&*/ ( dist < min_dist_around_soma) )
	 {
	    //std::cout<< " node number " << j <<std::endl;
	    nodes_near_soma.push_back(vertices->at(j)->coordinates); 
	    
	    if(vertices->at(j)->distanceFromSoma == 0)
	    {
		// call the function to begin traversing the que iteratively and set the distances from soma
		unprocessed_vertex_que.push(vertices->at(j));
		std::cout<< "from main pushed vertex number "<< j << std::endl;
		std::cout<< " pushed vertex "<< vertices->at(j)->coordinates[0]<< ", " << vertices->at(j)->coordinates[1]<< ", "<< vertices->at(j)->coordinates[2]<< std::endl;
	        push_cnt++;
		main_push_cnt++;
		pushed_stack.push_back(vertices->at(j));
		// call the recursive function.. this time with distance value from soma
		setDistanceToVertex(i,dist);
      ...
      // test code
      // find the coordinates that are not pushed
      for (int k = 0; k < present_graph->getNumberOfVertices(); k++)
      {
	
	if(!foundInPushedStack(vertices->at(k)->coordinates))
	{
	  
	  std::cout<< "Missing points "<< vertices->at(k)->coordinates[0]<<", "<< vertices->at(k)->coordinates[1]<<", "<< vertices->at(k)->coordinates[2]<<"\n ";
	  
	}
	
      }
      
      //std::cout<< "Nodes near soma " <<std::endl;
      
      /*for(int k = 0; k < nodes_near_soma.size(); k++)
      {
	 std::cout<< nodes_near_soma.at(k)[0] <<" "<< nodes_near_soma.at(k)[1] <<" "<<nodes_near_soma.at(k)[2] <<" "<<std::endl; 
      }*/
      
      // for each neareast node start traversing the path unidirectionally
      
	// go to the next connected node, set the distance
	
	// 
    
   }
   
    std::cout<< "main_push_cnt "<< main_push_cnt<<std::endl;
    std::cout<< "push_cnt "<< push_cnt<<std::endl;
...
bool AxonDendriteProximityFinder::foundInPushedStack(double * coordinate)
{
...
void AxonDendriteProximityFinder::setDistanceToVertex(int cell_index, double dist)
{
   ...
   
   {
     ...
     Vertex * curr_vertex = unprocessed_vertex_que.front();
      ...
      std::cout<< curr_vertex->coordinates[0]<< ", " << curr_vertex->coordinates[1]<< ", "<< curr_vertex->coordinates[2]<< std::endl;
	  
     unprocessed_vertex_que.pop();
     pop_count++;
     // set distance from soma to this node
     curr_vertex->distanceFromSoma = dist;
...
//      std::cout<< " connectedEdgesPtr->size() " << connectedEdgesPtr->size()<< std::endl;
     ...
     for( int i = 0; i < connectedEdgesPtr->size(); i++)
     {
        ...
        //  other vertex of this edge which is not yet processed
        int prob_vertex_num, real_vertex_num;
	prob_vertex_num = connectedEdgesPtr->at(i)->edgeConnectivity[1];
	
	if( (vertices->at(prob_vertex_num)->coordinates[0] == curr_vertex->coordinates[0]) &&
	    (vertices->at(prob_vertex_num)->coordinates[1] == curr_vertex->coordinates[1]) &&
	    (vertices->at(prob_vertex_num)->coordinates[2] == curr_vertex->coordinates[2]) )
	{
	   // this is the same vertex that we are currently processing, set vertex to be the other guy
	   real_vertex_num = connectedEdgesPtr->at(i)->edgeConnectivity[0];
	}
	else
	{
	   real_vertex_num = connectedEdgesPtr->at(i)->edgeConnectivity[1];
	}
	
 	//std::cout<< " edgeConnectivity[0]: "<<connectedEdgesPtr->at(i)->edgeConnectivity[0]<< " edgeConnectivity[1]: "<<connectedEdgesPtr->at(i)->edgeConnectivity[1]<<std::endl;
 	//std::cout<< " vertex number "<< real_vertex_num << "edge num "<< i <<std::endl;
	
	
	
	
        if( vertices->at(real_vertex_num)->distanceFromSoma == 0 )
	{
	  // push to the process queue
	  unprocessed_vertex_que.push(vertices->at(real_vertex_num));
	  push_cnt++;
 	  std::cout<< " pushed vertex number "<< real_vertex_num << std::endl;
 	  std::cout<< " pushed vertex "<< vertices->at(real_vertex_num)->coordinates[0]<< ", " << vertices->at(real_vertex_num)->coordinates[1]<< ", "<< vertices->at(real_vertex_num)->coordinates[2]<< std::endl;
 	  pushed_stack.push_back(vertices->at(real_vertex_num));
	}
	// for the current vertex set the distance by
	// adding the number of points to the least vertex distance found
	else if( (vertices->at(real_vertex_num)->distanceFromSoma < min_dist)  )
	{
	   min_dist_edge = connectedEdgesPtr->at(i);
	   min_dist = vertices->at(real_vertex_num)->distanceFromSoma;
	}
       
     }
     
     int num_of_edge_points = min_dist_edge->numEdgePoints;
     //std::cout<< "num_of_edge_points " << num_of_edge_points<< std::endl;
     //curr_vertex->distanceFromSoma = min_dist + min_dist_edge->numEdgePoints * SPATIAL_GRAPH_SAMPLING_DISTANCE;
     
     std::cout<< "curr_vertex->distanceFromSoma " << curr_vertex->distanceFromSoma << std::endl;
     
     setDistanceToVertex(cell_index);
   }
...
void AxonDendriteProximityFinder::setDistanceToVertex(int cell_index)
{
...
void AxonDendriteProximityFinder::writeGivenSpatialGraphFile(const char* filename, AmiraSpatialGraph * given_spatial_graph)
{
...
void AxonDendriteProximityFinder::findOppositionsBetweenAxonDendrite(AmiraSpatialGraph * cell1, AmiraSpatialGraph * cell2, std::vector<Proximity *>* ProxCelSpecificArray )
{
    std::vector< Edge * > * axonEdges = cell1->edgesPointer();
    std::vector< Edge * > * dendriteEdges = cell2->edgesPointer();
    std::vector< Vertex * > * axonVertices = cell1->verticesPointer();
    std::vector< Vertex * > * dendriteVertices = cell2->verticesPointer();
    
    // For each axon-dendrite pairing
//#pragma omp parallel for
    for(int axonEdgeCount = 0; axonEdgeCount < axonEdges->size(); axonEdgeCount++) 
    //if((axonEdgeCount == 19) || (axonEdgeCount == 660) || (axonEdgeCount == 318) || (axonEdgeCount == 364))
    {
        if(isAxon(axonEdges->at(axonEdgeCount)->label))
        {
            // for each axon edge, get its boundig box and see if there are any dendrite edge going near it
            Edge * currentAxon = axonEdges->at(axonEdgeCount);
            int AxonID = currentAxon->label;
            int axon_vertex_num1 = currentAxon->edgeConnectivity[0];
            int axon_vertex_num2 = currentAxon->edgeConnectivity[1];
...
//#pragma omp parallel for                
                for(int dendriteEdgeCount = 0; dendriteEdgeCount < dendriteEdges->size(); dendriteEdgeCount++)
                //if(dendriteEdgeCount == 660 || dendriteEdgeCount == 19)
                {
                    if( 
                        isDendrite(dendriteEdges->at(dendriteEdgeCount)->label) || isSoma(dendriteEdges->at(dendriteEdgeCount)->label)
                        //|| 
                        //( isAxon(dendriteEdges->at(dendriteEdgeCount)->label) && ((axonEdges->at(axonEdgeCount)->label) != dendriteEdges->at(dendriteEdgeCount)->label) ) 
                      )
                    {
                        Edge * currentDendrite = dendriteEdges->at(dendriteEdgeCount);
...
//#pragma omp parallel for
                        for(dend_point_it = currentDendrite->edgePointCoordinates.begin(); dend_point_it != currentDendrite->edgePointCoordinates.end(); dend_point_it++)
                        {
                            ...
                            {   
                                ...
                                min_dend_point_edge = currentDendrite;
                            }
                ...
                //                                      //std::cout<< axonPoint[0] <<" "<<axonPoint[1] <<" "<< axonPoint[2]<< std::endl;
                            ...
                            // get the axon and dendrite distances from respective cell soma before pushing the 
                            double axonDistanceFromSoma, dendriteDistanceFromSoma;
                            
                            //std::cout<<"vertex ind 0 "<<currentAxon->edgeConnectivity[0] << "vertex ind 1 " << currentAxon->edgeConnectivity[1]<< std::endl;
                    
                            if( axonVertices->at( currentAxon->edgeConnectivity[0])->distanceFromSoma < axonVertices->at( currentAxon->edgeConnectivity[1])->distanceFromSoma )
                            {
                            axonDistanceFromSoma = axonVertices->at( currentAxon->edgeConnectivity[0])->distanceFromSoma + 
                                        Utility::euclidean_distance( axonVertices->at(currentAxon->edgeConnectivity[0])->coordinates, axonPoint, 3 ,1);
                            }
                            
                            else
                            {
                            axonDistanceFromSoma = axonVertices->at( currentAxon->edgeConnectivity[1])->distanceFromSoma + 
                                        Utility::euclidean_distance( axonVertices->at(currentAxon->edgeConnectivity[1])->coordinates, axonPoint, 3 ,1);
                            
                            }
                            
                            if( dendriteVertices->at( min_dend_point_edge->edgeConnectivity[0])->distanceFromSoma < dendriteVertices->at( min_dend_point_edge->edgeConnectivity[1])->distanceFromSoma )
                            {
                            dendriteDistanceFromSoma = dendriteVertices->at( min_dend_point_edge->edgeConnectivity[0])->distanceFromSoma + 
                                        Utility::euclidean_distance( dendriteVertices->at(min_dend_point_edge->edgeConnectivity[0])->coordinates, min_dend_point, 3 ,1);
                            }
                            
                            else
                            {
                            dendriteDistanceFromSoma = dendriteVertices->at( min_dend_point_edge->edgeConnectivity[1])->distanceFromSoma + 
                                        Utility::euclidean_distance( dendriteVertices->at(min_dend_point_edge->edgeConnectivity[1])->coordinates, min_dend_point, 3 ,1);
                            
                            }
                            
                            //std::cout<< "axonDistanceFromSoma "<<" "<<axonVertices->at( currentAxon->edgeConnectivity[0])->distanceFromSoma <<"dendriteDistanceFromSoma "<< dendriteVertices->at( min_dend_point_edge->edgeConnectivity[0])->distanceFromSoma << std::endl;
                            
                            // find the min distance vertex for the current axon edge_it
                            
                            // add the current axon point distance to the node distance
                            //if(!Utility::samePoint(previous_prox_point,))
                            
                            Proximity * prox = new Proximity(axonPoint, min_dend_point, min_dist, currentAxon->label, min_dend_id, this->original_image,0, axonDistanceFromSoma, dendriteDistanceFromSoma);
                            //std::cout<< "prox axonDistanceFromSoma "<<" "<<prox->axonDistanceFromSoma <<"prox dendriteDistanceFromSoma "<< prox->dendriteDistanceFromSoma<< std::endl;
                            //std::cout<< "prox axon point "<<" "<<axonPoint[0]<<" "<<axonPoint[1]<<" "<<axonPoint[2]<<std::endl;
                            //std::cout<< "prox dend point "<<" "<<min_dend_point[0]<<" "<<min_dend_point[1]<<" "<<min_dend_point[2]<<std::endl;
                            ...
                            ProxCelSpecificArray->push_back(prox);
...
void AxonDendriteProximityFinder::findProximities1(void)
{
...
//#pragma omp parallel for
	...
	
        {
            ...
            //if(i==0 && j==0)
            {
	        std::vector< Edge * > * cell_1_edges = cell_specific_spatial_graps_vector.at(i)->edgesPointer();
            std::vector< Edge * > * cell_2_edges = cell_specific_spatial_graps_vector.at(j)->edgesPointer();
    // 		std::vector< Vertex * > * axonVertices = cell_specific_axon_vertices_list.at(i);
    // 		std::vector< Vertex * > * dendriteVertices = cell_specific_dendrite_vertices_list.at(j);
    //                 std::vector< Edge * > * axonEdges = cell_specific_axon_edges_list.at(i);
    // 		std::vector< Edge * > * dendriteEdges = cell_specific_dendrite_edges_list.at(j);
            
            //findOppositionsBetweenAxonDendrite(axonVertices,axonEdges, dendriteVertices, dendriteEdges, &ProxLinearArray );
            findOppositionsBetweenAxonDendrite(cell_specific_spatial_graps_vector.at(i),cell_specific_spatial_graps_vector.at(j),  &ProxLinearArray );
            }
        }
    }
    ...
    for(int i = 0; i < ProxLinearArray.size(); i++)
    {
       //std::cout<< " From Prox Array: "<<ProxLinearArray.at(i)->axonDistanceFromSoma<< " "<<ProxLinearArray.at(i)->axonDistanceFromSoma<< std::endl;
    ...
    RemoveDuplicateProximities(&ProxLinearArrayUnSorted);
    //ProxLinearArrayUnSorted = ProxLinearArray;
    
    //sortProxDist(&ProxLinearArrayUnSorted, &ProxLinearArraySorted, true);
    ProxLinearArraySorted = ProxLinearArrayUnSorted;
    
    std::string s1 = this->outputpath;
    s1+="/Proximities.csv";
    writeProxXls(s1.c_str(),&ProxLinearArraySorted);
    //writeProxXlsWithAxonDend(s1.c_str(),&ProxLinearArraySorted);
    std::string s2 = this->outputpath;
    s2+="/Proximity_Landmarks.landmarkAscii";
    writeProxLandmarks(s2.c_str(), &ProxLinearArraySorted);
    std::string s3 = this->outputpath;
    s3+="/Axon_Landmarks.landmarkAscii";
    writeProxAxonLandmarks(s3.c_str(), &ProxLinearArraySorted);
    std::string s4 = this->outputpath;
    s4+="/Dend_Landmarks.landmarkAscii";
    writeProxDendLandmarks(s4.c_str(), &ProxLinearArraySorted);
    
    
    std::string masterhxfilename(this->outputpath);
    
    masterhxfilename += "/Load_All_Proximity_Images.hx";
    ...
    int block_count = 0;
    for(int i = 0; i < ProxLinearArraySorted.size(); i++)
    {
      
        MasterHxData<<"load ${SCRIPTDIR}/Proximity_Zone_"<<i+1<<"/Load_Proximity_Zone_"<<i+1<<".hx"<<std::endl;
	
	
	if( (((i+1) % PROX_PER_BLOCK) == 0 ) || (i == (ProxLinearArraySorted.size()-1)) ) 
	{
	   int block_start_ind = (block_count * PROX_PER_BLOCK);
	   block_count++;
	   
	   std::string blockhxfilename(this->outputpath);
           blockhxfilename += "/Load_Proximity_Images_";
	   blockhxfilename += Utility::inttochar(block_start_ind+1);
	   blockhxfilename += "-";
	   blockhxfilename += Utility::inttochar(i+1);
	   blockhxfilename += ".hx";
       
       std::string blockhxfilename_non_decon(this->outputpath);
           blockhxfilename_non_decon += "/Load_Proximity_Images_";
	   blockhxfilename_non_decon += Utility::inttochar(block_start_ind+1);
	   blockhxfilename_non_decon += "-";
	   blockhxfilename_non_decon += Utility::inttochar(i+1);
       blockhxfilename_non_decon += "_non_decon";
	   blockhxfilename_non_decon += ".hx";
	   
	   std::ofstream BlockHxData(blockhxfilename.c_str());
       std::ofstream BlockHxData_non_decon(blockhxfilename_non_decon.c_str());
	   
	   for(int k = block_start_ind; k <= i; k++)
	   {
	     
	     BlockHxData<<"load ${SCRIPTDIR}/Proximity_Zone_"<<k+1<<"/Load_Proximity_Zone_"<<k+1<<".hx"<<std::endl;
         BlockHxData_non_decon<<"load ${SCRIPTDIR}/Proximity_Zone_"<<k+1<<"_non_decon"<<"/Load_Proximity_Zone_"<<k+1<<".hx"<<std::endl;
	     
	   }
	   
	   BlockHxData.close();
       BlockHxData_non_decon.close();
    ...
    // to create cell specific prox lists files
    for(int i = 0; i < num_of_cells; i++)
    {
        for(int j = 0; j < num_of_cells; j++)
        {
            //int i = 0; 
	    //int j = 1;
	    if(/*(i!=j) &&*/ (!ProxLinearArraySorted.empty()))
            {
	       
	       for(int k = 0; k < ProxLinearArraySorted.size(); k++)
	       {
		  if( ( amira_graph->getAxonCellIndex(ProxLinearArraySorted.at(k)->getAxonID()) == i) && ( amira_graph->getDendriteCellIndex( ProxLinearArraySorted.at(k)->getDendriteID() ) == j))
		  {
		      ProxMasterArrayCleanedSorted[i][j]->push_back(  new Proximity(ProxLinearArraySorted.at(k), this->original_image));
    ...
    std::string cellspecific_proxcount_filename(this->outputpath);
    
    cellspecific_proxcount_filename += "/CellSpecificProxCount.csv";
    
    std::ofstream CellCountData(cellspecific_proxcount_filename.c_str());
    ...
    for(int i = 0; i < num_of_cells; i++)
    {
	    ...
	    CellCountData<< ProxMasterArrayCleanedSorted[i][j]->size() << ",";
	    
	    
	}
	CellCountData<< std::endl;
    }
	      
    CellCountData.close();      
	      
    std::string axoncellspecific_avg_dist_from_soma_filename(this->outputpath);
    
    axoncellspecific_avg_dist_from_soma_filename += "/CellSpecificAvgProxDistaceAxon.csv";
    
    std::ofstream axonAvgProxDistData(axoncellspecific_avg_dist_from_soma_filename.c_str());
    ...
    for(int i = 0; i < num_of_cells; i++)
    {
        for(int j = 0; j < num_of_cells; j++)
        {
	    double cell_specific_prox_dist_sum = 0;
	    // write the cell count
	    for(int k =0; k < ProxMasterArrayCleanedSorted[i][j]->size(); k++)
	    {
	      cell_specific_prox_dist_sum += ProxMasterArrayCleanedSorted[i][j]->at(k)->axonDistanceFromSoma;
	    }
	    
	    
	    
	    axonAvgProxDistData<< (cell_specific_prox_dist_sum /ProxMasterArrayCleanedSorted[i][j]->size()) << ",";
	    
	    
	}
	axonAvgProxDistData<< std::endl;
    }
	      
    axonAvgProxDistData.close(); 
    
    std::string dendcellspecific_avg_dist_from_soma_filename(this->outputpath);
    
    dendcellspecific_avg_dist_from_soma_filename += "/CellSpecificAvgProxDistaceDendrite.csv";
    
    std::ofstream dendAvgProxDistData(dendcellspecific_avg_dist_from_soma_filename.c_str());
    ...
    for(int i = 0; i < num_of_cells; i++)
    {
	    ...
	    for(int k =0; k < ProxMasterArrayCleanedSorted[i][j]->size(); k++)
	    {
	      cell_specific_prox_dist_sum += ProxMasterArrayCleanedSorted[i][j]->at(k)->dendriteDistanceFromSoma;
	    }
	    
	    
	    
	    dendAvgProxDistData<< (cell_specific_prox_dist_sum /ProxMasterArrayCleanedSorted[i][j]->size()) << ",";
	    
	    
	}
	dendAvgProxDistData<< std::endl;
    }
	      
    dendAvgProxDistData.close();   
    
    for(int i = 0; i < num_of_cells; i++)
    {
        for(int j = 0; j < num_of_cells; j++)
        {
            if(/*(i!=j) &&*/ (!ProxMasterArrayCleanedSorted[i][j]->empty()))
            {
                
               //findValleys(ProxMasterArray[i][j], ProxMasterArrayCleaned[i][j], this->max_dist, 1.0 );
               //sortProxDist(ProxMasterArrayCleaned[i][j], ProxMasterArrayCleanedSorted[i][j], false);
               ...
               std::string xlsfilename = format + "Proxlist.csv";
               ...
               writeProxLandmarks(landmarkfilename.c_str(), ProxMasterArrayCleanedSorted[i][j]);
                ...
                std::string cellspecifichxfilename = format +"_LoadProximityImages.hx";
    
                //std::cout<<cellspecifichxfilename<<std::endl;
                std::ofstream CellSpecificMasterHxData(cellspecifichxfilename.c_str());
                
                CellSpecificMasterHxData << "# Amira Script"<< std::endl;
                CellSpecificMasterHxData << " "<< std::endl;
                
		//std::cout<< "cell " << i << "to cell " << j  << std::endl; 
		
                for(int k = 0; k < ProxMasterArrayCleanedSorted[i][j]->size(); k++)
                {
		    //std::cout<< "cell " << i << "to cell " << j << "line id " << ProxMasterArrayCleanedSorted[i][j]->at(k)->getLinearID() << std::endl; 
                    CellSpecificMasterHxData<<"load ${SCRIPTDIR}/Proximity_Zone_"<< ProxMasterArrayCleanedSorted[i][j]->at(k)->getLinearID()<<"/Load_Proximity_Zone_"<<  ProxMasterArrayCleanedSorted[i][j]->at(k)->getLinearID() <<".hx"<<std::endl;
                }
                
                CellSpecificMasterHxData.close();
              //format += "_roi_";
...
//               }
    ...
    for(int i = 0; i < num_of_cells; i++)
    {
        for(int j = 0; j < num_of_cells; j++)
        {
            ...
            if(i != j)
            {
	        std::vector< Vertex * > * axonVertices = cell_specific_spatial_graps_vector.at(i)->verticesPointer();
		std::vector< Vertex * > * dendriteVertices = cell_specific_spatial_graps_vector.at(j)->verticesPointer();
		
		std::cout<< "got vertex pointers "<< std::endl;
                // For each axon-dendrite pairing
                for(int axonEdgeCount = 0; axonEdgeCount < cell_specific_axon_list.at(i)->size(); axonEdgeCount++)    
                {
                   
                        Edge * currentAxon = cell_specific_axon_list.at(i)->at(axonEdgeCount);
                        int AxonID = currentAxon->label;
                        std::list<double*>::iterator axon_point_it ;//= currentAxon->edgePointCoordinates;
                        for(axon_point_it = currentAxon->edgePointCoordinates.begin(); axon_point_it != currentAxon->edgePointCoordinates.end(); axon_point_it++)
                        {
				...
				Edge* min_dend_point_edge;
                                ...
                                for(int dendriteEdgeCount = 0; dendriteEdgeCount < cell_specific_dendrite_list.at(j)->size(); dendriteEdgeCount++)
                                {
                                    //if(isDendrite(edges->at(dendriteCount)->label) && (amira_graph->getDendriteCellIndex(edges->at(dendriteCount)->label) == j)) 
                                    {
                                        ...
                                        Edge * currentDendrite = cell_specific_dendrite_list.at(j)->at(dendriteEdgeCount);
...
//                                      //std::cout<< axonPoint[0] <<" "<<axonPoint[1] <<" "<< axonPoint[2]<< std::endl;
				    ...
				    std::cout<< "axonDistanceFromSoma "<<" "<<axonDistanceFromSoma <<"dendriteDistanceFromSoma "<< dendriteDistanceFromSoma<< std::endl;
    ...
    for(int i = 0; i < num_of_cells; i++)
    {
        for(int j = 0; j < num_of_cells; j++)
        {
	        ...
	        std::vector< Vertex * > * axonVertices = cell_specific_spatial_graps_vector.at(i)->verticesPointer();
		std::vector< Vertex * > * dendriteVertices = cell_specific_spatial_graps_vector.at(j)->verticesPointer();
                std::vector< Edge * > * axonEdges = cell_specific_spatial_graps_vector.at(i)->edgesPointer();
		std::vector< Edge * > * dendriteEdges = cell_specific_spatial_graps_vector.at(j)->edgesPointer();
                
		// For each axon-dendrite pairing
                for(int axonCount = 0; axonCount < axonEdges->size(); axonCount++)    
                {
                    ////std::cout<< edges->at(axonCount)->label<<std::endl;
                    if(isAxon(axonEdges->at(axonCount)->label))//&& (amira_graph->getAxonCellIndex(edges->at(axonCount)->label) == i))
                    {
                        Edge * currentAxon = axonEdges->at(axonCount);
                        int AxonID = currentAxon->label;
                        std::list<double*>::iterator axon_point_it ;//= currentAxon->edgePointCoordinates;
                        for(axon_point_it = currentAxon->edgePointCoordinates.begin(); axon_point_it != currentAxon->edgePointCoordinates.end(); axon_point_it++)
                        {
				...
				Edge* min_dend_point_edge;
                                ...
                                for(int dendriteCount = 0; dendriteCount < dendriteEdges->size(); dendriteCount++)
                                {
                                    if(isDendrite(dendriteEdges->at(dendriteCount)->label))// && (amira_graph->getDendriteCellIndex(edges->at(dendriteCount)->label) == j)) 
                                    {
                                        ////std::cout<<"dend edge index"<<dendriteCount<<std::endl;
                                        //SimpleDendrite currentDendrite = dendrite_list.at(dendriteCount);
                                        Edge * currentDendrite = dendriteEdges->at(dendriteCount);
...
//                                      //std::cout<< axonPoint[0] <<" "<<axonPoint[1] <<" "<< axonPoint[2]<< std::endl;
    ...
    */
    ...
    {
    std::string format = this->outputpath;
    format += "/1_2_test_landmark_locations.landmarkAscii";
    std::ofstream LandMarkData( format.c_str() );
    
    LandMarkData << "# AmiraMesh 3D ASCII 2.0"            << std::endl;
    LandMarkData << ""                                    << std::endl;
    LandMarkData << ""                                    << std::endl;
    LandMarkData << "define Markers " << ProxMasterArrayCleaned[0][1]->size()*3                << std::endl;
    LandMarkData << ""                                    << std::endl;
    LandMarkData << "Parameters {"                        << std::endl;
    LandMarkData << "    NumSets 1,"                      << std::endl;
    LandMarkData << "    ContentType \"LandmarkSet\""     << std::endl;
    ...
    for(it = ProxMasterArrayCleaned[0][1]->begin(); it != ProxMasterArrayCleaned[0][1]->end() ; it++)
    {
        LandMarkData << (*it)->getRealCentroid()[X_COORD] << " " << (*it)->getRealCentroid()[Y_COORD] << " " << (*it)->getRealCentroid()[Z_COORD] << std::endl;
        LandMarkData << (*it)->getAxonPoint()[X_COORD] << " " << (*it)->getAxonPoint()[Y_COORD] << " " << (*it)->getAxonPoint()[Z_COORD] << std::endl;
        LandMarkData << (*it)->getDendritePoint()[X_COORD] << " " << (*it)->getDendritePoint()[Y_COORD] << " " << (*it)->getDendritePoint()[Z_COORD] << std::endl;
        
    }

    //std::cout<< "1 2"<<//std::cout<<std::endl;
    LandMarkData.close();
    }
    ...
    std::string format = this->outputpath;
    format += "/2_1_test_landmark_locations.landmarkAscii";
    std::ofstream LandMarkData( format.c_str() );
    
    LandMarkData << "# AmiraMesh 3D ASCII 2.0"            << std::endl;
    LandMarkData << ""                                    << std::endl;
    LandMarkData << ""                                    << std::endl;
    LandMarkData << "define Markers " << ProxMasterArrayCleaned[1][0]->size()*3                << std::endl;
    LandMarkData << ""                                    << std::endl;
    LandMarkData << "Parameters {"                        << std::endl;
    LandMarkData << "    NumSets 1,"                      << std::endl;
    LandMarkData << "    ContentType \"LandmarkSet\""     << std::endl;
    ...
    for(it = ProxMasterArrayCleaned[1][0]->begin(); it != ProxMasterArrayCleaned[1][0]->end() ; it++)
    {
        LandMarkData << (*it)->getRealCentroid()[X_COORD] << " " << (*it)->getRealCentroid()[Y_COORD] << " " << (*it)->getRealCentroid()[Z_COORD] << std::endl;
        LandMarkData << (*it)->getAxonPoint()[X_COORD] << " " << (*it)->getAxonPoint()[Y_COORD] << " " << (*it)->getAxonPoint()[Z_COORD] << std::endl;
        LandMarkData << (*it)->getDendritePoint()[X_COORD] << " " << (*it)->getDendritePoint()[Y_COORD] << " " << (*it)->getDendritePoint()[Z_COORD] << std::endl;
    ...
    //std::cout<< "2 1"<<//std::cout<<std::endl;
   
   {
    
    std::string format = this->outputpath;
    format += "/1_3_test_landmark_locations.landmarkAscii";
    std::ofstream LandMarkData( format.c_str() );
    
    LandMarkData << "# AmiraMesh 3D ASCII 2.0"            << std::endl;
    LandMarkData << ""                                    << std::endl;
    LandMarkData << ""                                    << std::endl;
    LandMarkData << "define Markers " << ProxMasterArrayCleaned[0][2]->size() *3               << std::endl;
    LandMarkData << ""                                    << std::endl;
    LandMarkData << "Parameters {"                        << std::endl;
    LandMarkData << "    NumSets 1,"                      << std::endl;
    LandMarkData << "    ContentType \"LandmarkSet\""     << std::endl;
    ...
    for(it = ProxMasterArrayCleaned[0][2]->begin(); it != ProxMasterArrayCleaned[0][2]->end() ; it++)
    {
        LandMarkData << (*it)->getRealCentroid()[X_COORD] << " " << (*it)->getRealCentroid()[Y_COORD] << " " << (*it)->getRealCentroid()[Z_COORD] << std::endl;
        LandMarkData << (*it)->getAxonPoint()[X_COORD] << " " << (*it)->getAxonPoint()[Y_COORD] << " " << (*it)->getAxonPoint()[Z_COORD] << std::endl;
        LandMarkData << (*it)->getDendritePoint()[X_COORD] << " " << (*it)->getDendritePoint()[Y_COORD] << " " << (*it)->getDendritePoint()[Z_COORD] << std::endl;
    ...
    //std::cout<< "1 3"<<//std::cout<<std::endl;
    
    {
    std::string format = this->outputpath;
    format += "/3_1_test_landmark_locations.landmarkAscii";
    std::ofstream LandMarkData( format.c_str() );
    
    LandMarkData << "# AmiraMesh 3D ASCII 2.0"            << std::endl;
    LandMarkData << ""                                    << std::endl;
    LandMarkData << ""                                    << std::endl;
    LandMarkData << "define Markers " << ProxMasterArrayCleaned[2][0]->size()   *3             << std::endl;
    LandMarkData << ""                                    << std::endl;
    LandMarkData << "Parameters {"                        << std::endl;
    LandMarkData << "    NumSets 1,"                      << std::endl;
    LandMarkData << "    ContentType \"LandmarkSet\""     << std::endl;
    ...
    for(it = ProxMasterArrayCleaned[2][0]->begin(); it != ProxMasterArrayCleaned[2][0]->end() ; it++)
    {
        LandMarkData << (*it)->getRealCentroid()[X_COORD] << " " << (*it)->getRealCentroid()[Y_COORD] << " " << (*it)->getRealCentroid()[Z_COORD] << std::endl;
        LandMarkData << (*it)->getAxonPoint()[X_COORD] << " " << (*it)->getAxonPoint()[Y_COORD] << " " << (*it)->getAxonPoint()[Z_COORD] << std::endl;
        LandMarkData << (*it)->getDendritePoint()[X_COORD] << " " << (*it)->getDendritePoint()[Y_COORD] << " " << (*it)->getDendritePoint()[Z_COORD] << std::endl;
    ...
    //std::cout<< "3 1"<<//std::cout<<std::endl;
    
    {
    std::string format = this->outputpath;
    format += "/2_3_test_landmark_locations.landmarkAscii";
    std::ofstream LandMarkData( format.c_str() );
    
    LandMarkData << "# AmiraMesh 3D ASCII 2.0"            << std::endl;
    LandMarkData << ""                                    << std::endl;
    LandMarkData << ""                                    << std::endl;
    LandMarkData << "define Markers " << ProxMasterArrayCleaned[1][2]->size()  *3              << std::endl;
    LandMarkData << ""                                    << std::endl;
    LandMarkData << "Parameters {"                        << std::endl;
    LandMarkData << "    NumSets 1,"                      << std::endl;
    LandMarkData << "    ContentType \"LandmarkSet\""     << std::endl;
    ...
    for(it = ProxMasterArrayCleaned[1][2]->begin(); it != ProxMasterArrayCleaned[1][2]->end() ; it++)
    {
        LandMarkData << (*it)->getRealCentroid()[X_COORD] << " " << (*it)->getRealCentroid()[Y_COORD] << " " << (*it)->getRealCentroid()[Z_COORD] << std::endl;
        LandMarkData << (*it)->getAxonPoint()[X_COORD] << " " << (*it)->getAxonPoint()[Y_COORD] << " " << (*it)->getAxonPoint()[Z_COORD] << std::endl;
        LandMarkData << (*it)->getDendritePoint()[X_COORD] << " " << (*it)->getDendritePoint()[Y_COORD] << " " << (*it)->getDendritePoint()[Z_COORD] << std::endl;
    ...
    //std::cout<< "2 3"<<//std::cout<<std::endl;
    
    {
        
    std::string format = this->outputpath;
    format += "/3_2_test_landmark_locations.landmarkAscii";
    std::ofstream LandMarkData( format.c_str() );
    
    LandMarkData << "# AmiraMesh 3D ASCII 2.0"            << std::endl;
    LandMarkData << ""                                    << std::endl;
    LandMarkData << "define Markers " << ProxMasterArrayCleaned[2][1]->size()   *3             << std::endl;
    LandMarkData << ""                                    << std::endl;
    LandMarkData << "Parameters {"                        << std::endl;
    LandMarkData << "    NumSets 1,"                      << std::endl;
    LandMarkData << "    ContentType \"LandmarkSet\""     << std::endl;
    ...
    for(it = ProxMasterArrayCleaned[2][1]->begin(); it != ProxMasterArrayCleaned[2][1]->end() ; it++)
    {
        LandMarkData << (*it)->getRealCentroid()[X_COORD] << " " << (*it)->getRealCentroid()[Y_COORD] << " " << (*it)->getRealCentroid()[Z_COORD] << std::endl;
        LandMarkData << (*it)->getAxonPoint()[X_COORD] << " " << (*it)->getAxonPoint()[Y_COORD] << " " << (*it)->getAxonPoint()[Z_COORD] << std::endl;
        LandMarkData << (*it)->getDendritePoint()[X_COORD] << " " << (*it)->getDendritePoint()[Y_COORD] << " " << (*it)->getDendritePoint()[Z_COORD] << std::endl;
    ...
    //std::cout<< "3 2"<<//std::cout<<std::endl;
    
    
    
    //std::cout<< "wrote file"<<//std::cout<<std::endl;
#endif    
};
...
bool AxonDendriteProximityFinder::isTheSameCell(int AxonID, int DendriteID)
{
    ...
    else if(amira_graph->getAxonCellIndex(AxonID) == amira_graph->getSomaCellIndex(DendriteID))
    {
        return true;
    }
    else
...
bool AxonDendriteProximityFinder::isSoma(int ID)
{
