...
void BoutonFinder::getAxonPoints(ImageType::Pointer image)
{
    ...
    unsigned int numOfEdges = edges->size();
    unsigned int end_x = rint(image->GetLargestPossibleRegion().GetSize(0)*XYSAMPLING);
    unsigned int end_y = rint(image->GetLargestPossibleRegion().GetSize(1)*XYSAMPLING);
    unsigned int end_z = rint(image->GetLargestPossibleRegion().GetSize(2)*ZSAMPLING);
