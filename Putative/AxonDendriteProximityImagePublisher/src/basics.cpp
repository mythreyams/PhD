...
void AmiraSpatialGraph::setTransformation(double ** newTransform)
{
	for(int ii = 0; ii < 4; ++ii)
		for(int jj = 0; jj < 4; ++jj)
			this->transformation[ii][jj] = newTransform[ii][jj];
	...
	for(int ii = 0; ii < 4; ++ii)
		...
		{
			if(ii != jj)
				if(fabs(this->transformation[ii][jj]) > 1E-6)
				{
					isId = 0;
// 					//////std::cout << "T[" << ii << "][" << jj << "] = " << transformation[ii][jj] << std::endl;
				...
				if(fabs(this->transformation[ii][jj] - 1) > 1E-6)
				{
					isId = 0;
// 					//////std::cout << "T[" << ii << "][" << jj << "] = " << transformation[ii][jj] << std::endl;
				}
...
{
	if(!this->isIdentity && !this->transformationApplied)
	{
// 		printTransformation();
		this->transformationApplied = 1;
...
// 			++vertexCount;
			...
			for(int ii = 0; ii < 3; ++ii)
				for(int jj = 0; jj < 4; ++jj)
					newCoords[ii] += this->transformation[ii][jj]*oldCoords[jj];
